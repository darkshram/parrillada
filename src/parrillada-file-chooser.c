...
static void parrillada_file_chooser_class_init (ParrilladaFileChooserClass *klass);
static void parrillada_file_chooser_init (ParrilladaFileChooser *sp);
static void parrillada_file_chooser_iface_uri_container_init (ParrilladaURIContainerIFace *iface);
static void parrillada_file_chooser_iface_layout_object_init (ParrilladaLayoutObjectIFace *iface);
static void parrillada_file_chooser_finalize (GObject *object);

static void
parrillada_file_chooser_uri_activated_cb (GtkFileChooser *widget,
				       ParrilladaFileChooser *chooser);
static void
parrillada_file_chooser_uri_selection_changed_cb (GtkFileChooser *widget,
					       ParrilladaFileChooser *chooser);
struct ParrilladaFileChooserPrivate {
	GtkWidget *chooser;
...
GType
parrillada_file_chooser_get_type ()
{
	static GType type = 0;

	if(type == 0) {
		static const GTypeInfo our_info = {
			sizeof (ParrilladaFileChooserClass),
			NULL,
			NULL,
			(GClassInitFunc)parrillada_file_chooser_class_init,
			NULL,
			NULL,
			sizeof (ParrilladaFileChooser),
			0,
			(GInstanceInitFunc)parrillada_file_chooser_init,
		};

		static const GInterfaceInfo uri_container_info =
		{
			(GInterfaceInitFunc) parrillada_file_chooser_iface_uri_container_init,
			NULL,
			NULL
		};
		...
		{
			(GInterfaceInitFunc) parrillada_file_chooser_iface_layout_object_init,
			NULL,
			NULL
		};
...
static void
parrillada_file_chooser_class_init (ParrilladaFileChooserClass *klass)
{
	GObjectClass *object_class = G_OBJECT_CLASS(klass);

	parent_class = g_type_class_peek_parent(klass);
	object_class->finalize = parrillada_file_chooser_finalize;
}

static void
parrillada_file_chooser_position_percent (GObject *object,
                                       gint width,
                                       gint position)
{
	gint percent;
...
static void
parrillada_file_chooser_position_changed (GObject *object,
                                       ...
                                       gpointer NULL_data)
{
	...
	parrillada_file_chooser_position_percent (object, allocation.width, position);
}

static void
parrillada_file_chooser_allocation_changed (GtkWidget *widget,
                                         ...
                                         gpointer NULL_data)
{
	...
	if (GPOINTER_TO_INT (g_object_get_data (G_OBJECT (widget), "position_set")) == FALSE) {
		...
		g_signal_connect (widget,
		                  "notify::position",
		                  G_CALLBACK (parrillada_file_chooser_position_changed),
		                  NULL);
		return;
	}
	...
	parrillada_file_chooser_position_percent (G_OBJECT (widget), width, position);
}

static void
parrillada_file_chooser_notify_model (GtkTreeView *treeview,
                                   GParamSpec *pspec,
                                   gpointer NULL_data)
{
	GtkTreeModel *model;
	...
	if (model && !EGG_IS_TREE_MULTI_DRAG_SOURCE (model)) {
		...
		parrillada_enable_multi_DND_for_model_type (type);
	}
}

void
parrillada_file_chooser_customize (GtkWidget *widget, gpointer null_data)
{
	...
	if (GTK_IS_TREE_VIEW (widget)) {
		...
		&&  gtk_tree_selection_get_mode (gtk_tree_view_get_selection (GTK_TREE_VIEW (widget))) == GTK_SELECTION_MULTIPLE) {
			...
			if (model) {
				...
				g_signal_connect (widget,
				                  "notify::model",
				                  G_CALLBACK (parrillada_file_chooser_notify_model),
				                  NULL);
		}
	}
		...
		if (GTK_IS_PANED (widget)) {
			...
			g_signal_connect (widget,
			                  "size-allocate",
			                  G_CALLBACK (parrillada_file_chooser_allocation_changed),
			                  NULL);
		}

		gtk_container_foreach (GTK_CONTAINER (widget),
				       parrillada_file_chooser_customize,
				       NULL);
	}
}

static void
parrillada_file_chooser_init (ParrilladaFileChooser *obj)
{
	GtkFileFilter *filter;

	obj->priv = g_new0 (ParrilladaFileChooserPrivate, 1);
	...
	g_signal_connect (obj->priv->chooser,
			  "file-activated",
			  G_CALLBACK (parrillada_file_chooser_uri_activated_cb),
			  obj);
	g_signal_connect (obj->priv->chooser,
			  "selection-changed",
			  G_CALLBACK (parrillada_file_chooser_uri_selection_changed_cb),
			  obj);
...
static void
parrillada_file_chooser_finalize (GObject *object)
{
	ParrilladaFileChooser *cobj;

	cobj = PARRILLADA_FILE_CHOOSER (object);
	g_free (cobj->priv);
...
GtkWidget *
parrillada_file_chooser_new ()
{
	gpointer *obj;
	
	obj = g_object_new (PARRILLADA_TYPE_FILE_CHOOSER, NULL);
	
	return GTK_WIDGET (obj);
}

static void
parrillada_file_chooser_find_pane (GtkWidget *child,
				gpointer footer)
{
	if (GTK_IS_PANED (child)) {
		...
		gtk_container_foreach (GTK_CONTAINER (child),
				       parrillada_file_chooser_find_pane,
				       footer);
	}
}
...
parrillada_file_chooser_uri_activated_cb (GtkFileChooser *widget,
				       ParrilladaFileChooser *chooser)
{
	parrillada_uri_container_uri_activated (PARRILLADA_URI_CONTAINER (chooser));
}
...
parrillada_file_chooser_uri_selection_changed_cb (GtkFileChooser *widget,
					       ParrilladaFileChooser *chooser)
{
	parrillada_uri_container_uri_selected (PARRILLADA_URI_CONTAINER (chooser));
}

static gchar *
parrillada_file_chooser_get_selected_uri (ParrilladaURIContainer *container)
{
	ParrilladaFileChooser *chooser;

	chooser = PARRILLADA_FILE_CHOOSER (container);
	return gtk_file_chooser_get_uri (GTK_FILE_CHOOSER (chooser->priv->chooser));
}

static gchar **
parrillada_file_chooser_get_selected_uris (ParrilladaURIContainer *container)
{
	ParrilladaFileChooser *chooser;
	GSList *list, *iter;
	gchar **uris;
...
static void
parrillada_file_chooser_iface_uri_container_init (ParrilladaURIContainerIFace *iface)
{
	iface->get_selected_uri = parrillada_file_chooser_get_selected_uri;
	iface->get_selected_uris = parrillada_file_chooser_get_selected_uris;
}

static void
parrillada_file_chooser_set_context (ParrilladaLayoutObject *object,
				  ParrilladaLayoutType type)
{
	ParrilladaFileChooser *self;

	self = PARRILLADA_FILE_CHOOSER (object);
...
static void
parrillada_file_chooser_get_proportion (ParrilladaLayoutObject *object,
				     gint *header,
				     gint *center,
				     gint *footer)
{
...
static void
parrillada_file_chooser_iface_layout_object_init (ParrilladaLayoutObjectIFace *iface)
{
	iface->get_proportion = parrillada_file_chooser_get_proportion;
	iface->set_context = parrillada_file_chooser_set_context;
}
