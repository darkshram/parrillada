...
G_DEFINE_TYPE (ParrilladaPlayer, parrillada_player, GTK_TYPE_ALIGNMENT);

struct ParrilladaPlayerPrivate {
	GtkWidget *notebook;
...
typedef enum {
	...
	EOF_SIGNAL,
	LAST_SIGNAL
} ParrilladaPlayerSignalType;
static guint parrillada_player_signals [LAST_SIGNAL] = { 0 };

static GObjectClass *parent_class = NULL;

static void
parrillada_player_no_multimedia_stream (ParrilladaPlayer *player)
{
...
static void
parrillada_player_video_zoom_out (ParrilladaPlayer *player)
{
...
static void
parrillada_player_video_zoom_in (ParrilladaPlayer *player)
{
	player->priv->video_width += PLAYER_BACON_WIDTH / 3;
	player->priv->video_height += PLAYER_BACON_HEIGHT / 3;
...
static gboolean
parrillada_player_scale_image (ParrilladaPlayer *player)
{
	gint height, width;
	GdkPixbuf *scaled;
	gdouble ratio;
...
static void
parrillada_player_image_zoom_in (ParrilladaPlayer *player)
{
	...
	if (player->priv->pixbuf)
		parrillada_player_scale_image (player);
}

static void
parrillada_player_image_zoom_out (ParrilladaPlayer *player)
{
...
static void
parrillada_player_zoom_in_cb (GtkButton *button,
                           ParrilladaPlayer *player)
{
	parrillada_player_image_zoom_in (player);
	parrillada_player_video_zoom_in (player);
}

static void
parrillada_player_zoom_out_cb (GtkButton *button,
                            ParrilladaPlayer *player)
{
	parrillada_player_image_zoom_out (player);
	parrillada_player_video_zoom_out (player);
}

static gboolean
parrillada_bacon_scroll (ParrilladaPlayerBacon *bacon,
                      ...
                      ParrilladaPlayer *player)
{
	...
	case 4:
		if (event->direction == GDK_SCROLL_UP)
			parrillada_player_bacon_forward (bacon, GST_SECOND);
		else
			parrillada_player_bacon_backward (bacon, GST_SECOND);
		break;
...
static gboolean
parrillada_bacon_button_release (ParrilladaPlayerBacon *bacon,
                              ...
                              ParrilladaPlayer *player)
{
	...
	if (player->priv->state == BACON_STATE_READY) {
		...
		parrillada_player_bacon_set_uri (PARRILLADA_PLAYER_BACON (player->priv->bacon), player->priv->uri);
		parrillada_player_bacon_play (PARRILLADA_PLAYER_BACON (player->priv->bacon));
	}
		...
		parrillada_player_bacon_stop (PARRILLADA_PLAYER_BACON (player->priv->bacon));
	}
...
static gboolean
parrillada_player_button_release (GtkWidget *widget,
                               GdkEventButton *event,
                               ParrilladaPlayer *player)
{
	if (event->button != 1)
...
static gboolean
parrillada_player_scroll (GtkWidget *widget,
                       GdkEventScroll *event,
                       ParrilladaPlayer *player)
{
	switch (gtk_notebook_get_current_page (GTK_NOTEBOOK (player->priv->notebook))) {
	...
	case 4:
		if (event->direction == GDK_SCROLL_UP)
			parrillada_player_bacon_forward (PARRILLADA_PLAYER_BACON (player->priv->bacon), GST_SECOND);
		else
			parrillada_player_bacon_backward (PARRILLADA_PLAYER_BACON (player->priv->bacon), GST_SECOND);
		break;
...
static void
parrillada_player_image (ParrilladaPlayer *player)
{
	GError *error = NULL;
	gchar *path;
	...
	if (!player->priv->pixbuf) {
		...
		parrillada_player_no_multimedia_stream (player);
		g_free (path);
		return;
	}
	...
	g_signal_emit (player,
		       parrillada_player_signals [READY_SIGNAL],
		       0);
}

static void
parrillada_player_update_tooltip (ParrilladaPlayer *player,
                               GFileInfo *info)
{
	gchar *string;
	gchar *len_string;
	...
	/* Update the tooltip */
	len_string = parrillada_units_get_time_string (g_file_info_get_attribute_uint64 (info, PARRILLADA_IO_LEN), TRUE, FALSE);
	title = g_file_info_get_attribute_string (info, PARRILLADA_IO_TITLE);
	artist = g_file_info_get_attribute_string (info, PARRILLADA_IO_ARTIST);
	if (artist) {
		gchar *artist_string;
...
static void
parrillada_player_metadata_completed (GObject *obj,
				   GError *error,
				   const gchar *uri,
				   GFileInfo *info,
				   gpointer null_data)
{
	ParrilladaPlayer *player = PARRILLADA_PLAYER (obj);
	const gchar *mime;
	...
	if (g_file_info_get_attribute_boolean (info, PARRILLADA_IO_HAS_VIDEO)) {
		if (g_file_info_get_attribute_uint64 (info, PARRILLADA_IO_LEN) <= 0) {
			parrillada_player_no_multimedia_stream (player);
			g_signal_emit (player,
				       parrillada_player_signals [ERROR_SIGNAL],
				       0);
		...
		parrillada_player_update_tooltip (player, info);
	}
	else if (g_file_info_get_attribute_boolean (info, PARRILLADA_IO_HAS_AUDIO)) {
		if (g_file_info_get_attribute_uint64 (info, PARRILLADA_IO_LEN) <= 0) {
			parrillada_player_no_multimedia_stream (player);
			g_signal_emit (player,
				       parrillada_player_signals [ERROR_SIGNAL],
		...
		parrillada_player_image (player);

		path = g_filename_from_uri (player->priv->uri, NULL, NULL);
		PARRILLADA_GET_BASENAME_FOR_DISPLAY (path, name);
		g_free (path);
	...
	if (player->priv->end <= 0)
		player->priv->end = g_file_info_get_attribute_uint64 (info, PARRILLADA_IO_LEN);

	player->priv->length = g_file_info_get_attribute_uint64 (info, PARRILLADA_IO_LEN);
...
static void
parrillada_player_retrieve_metadata (ParrilladaPlayer *player)
{
	if (!player->priv->meta_task)
		player->priv->meta_task = parrillada_io_register (G_OBJECT (player),
							       parrillada_player_metadata_completed,
							       NULL,
							       NULL);

	parrillada_io_get_file_info (player->priv->uri,
			       ...
			       gint64 end)
{
	...
	if (player->priv->bacon)
		parrillada_player_bacon_set_boundaries (PARRILLADA_PLAYER_BACON (player->priv->bacon),
						     player->priv->start,
						     player->priv->end);
}
			...
			const gchar *uri)
{
	...
	if (player->priv->meta_task)
		parrillada_io_cancel_by_base (player->priv->meta_task);

	/* That stops the pipeline from playing */
	parrillada_player_bacon_set_uri (PARRILLADA_PLAYER_BACON (player->priv->bacon), NULL);
	if (!uri) {
		parrillada_player_no_multimedia_stream (player);
		return;
	}
	...
	parrillada_player_retrieve_metadata (player);
}

static void
parrillada_player_eof_cb (ParrilladaPlayerBacon *bacon, ParrilladaPlayer *player)
{
	parrillada_player_bacon_stop (PARRILLADA_PLAYER_BACON (player->priv->bacon));
	parrillada_player_bacon_set_pos (PARRILLADA_PLAYER_BACON (player->priv->bacon), player->priv->start);
	player->priv->state = BACON_STATE_PAUSED;
	gtk_image_set_from_stock (GTK_IMAGE (player->priv->play_image), GTK_STOCK_MEDIA_PLAY, GTK_ICON_SIZE_DIALOG);
}

static void
parrillada_player_state_changed_cb (ParrilladaPlayerBacon *bacon,
				 ParrilladaPlayerBaconState state,
				 ParrilladaPlayer *player)
{
...
parrillada_player_destroy (GtkObject *obj)
{
	...
	if (player->priv->meta_task){
		parrillada_io_cancel_by_base (player->priv->meta_task);
		parrillada_io_job_base_free (player->priv->meta_task);
...
static void
parrillada_player_finalize (GObject *object)
{
	ParrilladaPlayer *cobj;

	cobj = PARRILLADA_PLAYER (object);
...
parrillada_player_class_init (ParrilladaPlayerClass *klass)
{
	...
	parrillada_player_signals [ERROR_SIGNAL] = 
			g_signal_new ("error",
				      ...
				      G_SIGNAL_RUN_LAST,
				      G_STRUCT_OFFSET (ParrilladaPlayerClass, error),
				      NULL, NULL,
	...
	parrillada_player_signals [READY_SIGNAL] = 
			g_signal_new ("ready",
				      G_TYPE_FROM_CLASS (klass),
				      G_SIGNAL_RUN_LAST,
				      G_STRUCT_OFFSET (ParrilladaPlayerClass, ready),
				      NULL, NULL,
				      g_cclosure_marshal_VOID__VOID,
				      G_TYPE_NONE, 0);
}
...
static void
parrillada_player_volume_changed_cb (GtkScaleButton *button,
				  gdouble volume,
				  ParrilladaPlayer *player)
{
	parrillada_player_bacon_set_volume (PARRILLADA_PLAYER_BACON (player->priv->bacon), volume);
}

static void
parrillada_player_init (ParrilladaPlayer *obj)
{
	GtkWidget *volume;
	GtkWidget *image;
	GtkWidget *label;
	...
	obj->priv = g_new0 (ParrilladaPlayerPrivate, 1);
	...
	g_signal_connect (event,
			  ...
			  G_CALLBACK (parrillada_player_button_release),
			  obj);
	g_signal_connect (event,
			  "scroll-event",
			  G_CALLBACK (parrillada_player_scroll),
			  obj);
	...
	/* Video */
	obj->priv->bacon = parrillada_player_bacon_new ();
	gtk_widget_show (obj->priv->bacon);
	g_signal_connect (obj->priv->bacon,
			  "button-release-event",
			  G_CALLBACK (parrillada_bacon_button_release),
			  obj);
	g_signal_connect (obj->priv->bacon,
			  "scroll-event",
			  G_CALLBACK (parrillada_bacon_scroll),
			  obj);
	g_signal_connect (obj->priv->bacon,
			  "state-change",
			  G_CALLBACK (parrillada_player_state_changed_cb),
			  obj);
	g_signal_connect (obj->priv->bacon,
			  "eof",
			  G_CALLBACK (parrillada_player_eof_cb),
			  obj);
	...
	g_signal_connect (zoom,
			  "clicked",
			  G_CALLBACK (parrillada_player_zoom_out_cb),
			  obj);
	...
	g_signal_connect (zoom,
			  "clicked",
			  G_CALLBACK (parrillada_player_zoom_in_cb),
			  obj);
	gtk_box_pack_start (GTK_BOX (hbox),
	                    zoom,
	                    FALSE,
	...
	gtk_scale_button_set_value (GTK_SCALE_BUTTON (volume),
				    parrillada_player_bacon_get_volume (PARRILLADA_PLAYER_BACON (obj->priv->bacon)));

	g_signal_connect (volume,
			  "value-changed",
			  G_CALLBACK (parrillada_player_volume_changed_cb),
			  obj);
}

GtkWidget *
parrillada_player_new ()
{
	ParrilladaPlayer *obj;
