...
typedef struct _ParrilladaMultiSongPropsPrivate ParrilladaMultiSongPropsPrivate;
struct _ParrilladaMultiSongPropsPrivate
{
	GtkWidget *title;
	GtkWidget *artist;
	GtkWidget *composer;
...
#define PARRILLADA_MULTI_SONG_PROPS_PRIVATE(o)  (G_TYPE_INSTANCE_GET_PRIVATE ((o), PARRILLADA_TYPE_MULTI_SONG_PROPS, ParrilladaMultiSongPropsPrivate))


G_DEFINE_TYPE (ParrilladaMultiSongProps, parrillada_multi_song_props, GTK_TYPE_DIALOG);

void
parrillada_multi_song_props_set_rename_callback (ParrilladaMultiSongProps *self,
					      GtkTreeSelection *selection,
					      gint column_num,
					      ParrilladaRenameCallback callback)
{
	ParrilladaMultiSongPropsPrivate *priv;

	priv = PARRILLADA_MULTI_SONG_PROPS_PRIVATE (self);

	parrillada_rename_do (PARRILLADA_RENAME (priv->title),
			   selection,
			   column_num,
			   callback);
}
...
static gboolean
parrillada_multi_song_props_gap_output_cb (GtkSpinButton *spin,
					ParrilladaMultiSongProps *self)
{
...
static guint
parrillada_multi_song_props_gap_input_cb (GtkSpinButton *spin,
				       gdouble *val,
				       ParrilladaMultiSongProps *self)
{
	const gchar *text;
...
static gboolean
parrillada_multi_song_props_entry_focus_out (GtkEntry *entry,
					  GdkEventFocus *event,
					  gpointer NULL_data)
{
	const gchar *text;
...
static gboolean
parrillada_multi_song_props_entry_focus_in (GtkEntry *entry,
					 GdkEventFocus *event,
					 gpointer NULL_data)
{
	const gchar *text;
...
void
parrillada_multi_song_props_set_show_gap (ParrilladaMultiSongProps *self,
				       gboolean show)
{
	ParrilladaMultiSongPropsPrivate *priv;
...
static void
parrillada_multi_song_props_init (ParrilladaMultiSongProps *object)
{
	gchar *title;
	GtkWidget *box;
	GtkWidget *label;
	...
	priv = PARRILLADA_MULTI_SONG_PROPS_PRIVATE (object);
	...
	priv->title = parrillada_rename_new ();
	...
	frame = parrillada_utils_pack_properties (title, priv->title, NULL);
	g_free (title);
	...
	frame = parrillada_utils_pack_properties (title, table, NULL);
	g_free (title);
	...
	g_signal_connect (priv->artist,
			  "focus-in-event",
			  G_CALLBACK (parrillada_multi_song_props_entry_focus_in),
			  NULL);

	g_signal_connect (priv->artist,
			  "focus-out-event",
			  G_CALLBACK (parrillada_multi_song_props_entry_focus_out),
			  NULL);
	...
	frame = parrillada_utils_pack_properties (title, box, NULL);
	g_free (title);
	priv->gap_box = frame;
	...
	g_signal_connect (priv->gap,
			  "output",
			  G_CALLBACK (parrillada_multi_song_props_gap_output_cb),
			  object);
	g_signal_connect (priv->gap,
			  "input",
			  G_CALLBACK (parrillada_multi_song_props_gap_input_cb),
			  object);
...
static void
parrillada_multi_song_props_finalize (GObject *object)
{
	G_OBJECT_CLASS (parrillada_multi_song_props_parent_class)->finalize (object);
}

static void
parrillada_multi_song_props_class_init (ParrilladaMultiSongPropsClass *klass)
{
	GObjectClass *object_class = G_OBJECT_CLASS (klass);

	g_type_class_add_private (klass, sizeof (ParrilladaMultiSongPropsPrivate));

	object_class->finalize = parrillada_multi_song_props_finalize;
}

GtkWidget *
parrillada_multi_song_props_new (void)
{
	return g_object_new (PARRILLADA_TYPE_MULTI_SONG_PROPS, NULL);
}
