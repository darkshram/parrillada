...
typedef struct _ParrilladaRenamePrivate ParrilladaRenamePrivate;
struct _ParrilladaRenamePrivate
{
	GtkWidget *notebook;
	GtkWidget *combo;
...
#define PARRILLADA_RENAME_PRIVATE(o)  (G_TYPE_INSTANCE_GET_PRIVATE ((o), PARRILLADA_TYPE_RENAME, ParrilladaRenamePrivate))
...
static gchar *
parrillada_rename_insert_string (ParrilladaRename *self,
			      const gchar *name)
{
	ParrilladaRenamePrivate *priv;
	gboolean is_at_end;
...
static gchar *
parrillada_rename_delete_string (ParrilladaRename *self,
			      const gchar *name)
{
	ParrilladaRenamePrivate *priv;
	const gchar *text;
...
static gchar *
parrillada_rename_substitute_string (ParrilladaRename *self,
			 	  const gchar *name)
{
	ParrilladaRenamePrivate *priv;
	const gchar *joker;
...
static gchar *
parrillada_rename_number_string (ParrilladaRename *self,
			      const gchar *name)
{
	ParrilladaRenamePrivate *priv;
	const gchar *right;
...
static gchar *
parrillada_rename_sequence_string (ParrilladaRename *self,
                                const gchar *name,
                                guint item_num,
                                guint nb_items)
{
...
redo:
		switch (mode) {
		case 0:
			new_name = parrillada_rename_insert_string (self, name);
			break;
		case 1:
			new_name = parrillada_rename_delete_string (self, name);
			break;
		case 2:
			new_name = parrillada_rename_substitute_string (self, name);
			break;
		case 3:
			new_name = parrillada_rename_number_string (self, name);
			break;
		case 4:
			new_name = parrillada_rename_sequence_string (self, name, item_num, nb_items);
			break;
		default:
			new_name = NULL;
			break;
...
static void
parrillada_rename_type_changed (GtkComboBox *combo,
			     ParrilladaRename *self)
{
	ParrilladaRenamePrivate *priv;
...
static void
parrillada_rename_init (ParrilladaRename *object)
{
	ParrilladaRenamePrivate *priv;
	GtkWidget *combo;
	GtkWidget *entry;
	...
	g_signal_connect (priv->combo,
			  "changed",
			  G_CALLBACK (parrillada_rename_type_changed),
			  object);
	...
	/* Reminder: if this string happens to be used somewhere else in parrillada
...
static void
parrillada_rename_finalize (GObject *object)
{
	G_OBJECT_CLASS (parrillada_rename_parent_class)->finalize (object);
}

static void
parrillada_rename_class_init (ParrilladaRenameClass *klass)
{
	GObjectClass* object_class = G_OBJECT_CLASS (klass);

	g_type_class_add_private (klass, sizeof (ParrilladaRenamePrivate));

	object_class->finalize = parrillada_rename_finalize;
}

GtkWidget *
parrillada_rename_new (void)
{
	return g_object_new (PARRILLADA_TYPE_RENAME, NULL);
}
