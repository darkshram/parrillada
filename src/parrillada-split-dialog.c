...
#include "parrillada-song-control.h"
...
typedef struct _ParrilladaSplitDialogPrivate ParrilladaSplitDialogPrivate;
struct _ParrilladaSplitDialogPrivate
{
	GtkWidget *cut;
...
#define PARRILLADA_SPLIT_DIALOG_PRIVATE(o)  (G_TYPE_INSTANCE_GET_PRIVATE ((o), PARRILLADA_TYPE_SPLIT_DIALOG, ParrilladaSplitDialogPrivate))

G_DEFINE_TYPE (ParrilladaSplitDialog, parrillada_split_dialog, GTK_TYPE_DIALOG);

void
parrillada_split_dialog_set_uri (ParrilladaSplitDialog *self,
			      const gchar *uri,
                              const gchar *title,
                              const gchar *artist)
{
	...
	parrillada_song_control_set_uri (PARRILLADA_SONG_CONTROL (priv->player), uri);
	parrillada_song_control_set_info (PARRILLADA_SONG_CONTROL (priv->player), title, artist);
}

void
parrillada_split_dialog_set_boundaries (ParrilladaSplitDialog *self,
				     gint64 start,
				     gint64 end)
{
	ParrilladaSplitDialogPrivate *priv;
	...
	if (PARRILLADA_DURATION_TO_BYTES (start) % 2352)
		start += PARRILLADA_BYTES_TO_DURATION (2352 - (PARRILLADA_DURATION_TO_BYTES (start) % 2352));

	if (PARRILLADA_DURATION_TO_BYTES (end) % 2352)
		end += PARRILLADA_BYTES_TO_DURATION (2352 - (PARRILLADA_DURATION_TO_BYTES (end) % 2352));

	if (end - start < PARRILLADA_MIN_STREAM_LENGTH)
		return;
...
parrillada_split_dialog_get_slices (ParrilladaSplitDialog *self)
{
	...
	do {
		...
		slice = g_new0 (ParrilladaAudioSlice, 1);
...
static gboolean
parrillada_split_dialog_size_error (ParrilladaSplitDialog *self)
{
	GtkWidget *message;
...
static gboolean
parrillada_split_dialog_cut (ParrilladaSplitDialog *self,
			  gint64 pos,
			  gboolean warn)
{
	ParrilladaSplitDialogPrivate *priv;
	ParrilladaAudioSlice slice = {0,0};
	GtkTreeModel *model;
	GtkTreeIter child;
	GtkTreeIter iter;
	gchar *length_str;
	...
	if (PARRILLADA_DURATION_TO_BYTES (pos) % 2352)
		pos += PARRILLADA_BYTES_TO_DURATION (2352 - (PARRILLADA_DURATION_TO_BYTES (pos) % 2352));
	...
	if (!gtk_tree_model_get_iter_first (model, &iter)) {
		...
		if (priv->end <= 0)
			end = parrillada_song_control_get_length (PARRILLADA_SONG_CONTROL (priv->player));
		else
			end = priv->end;

		/* check that pos > 300 sectors ( == 4 sec ) */
		if (warn
		&&  pos - priv->start < PARRILLADA_MIN_STREAM_LENGTH
		&& !parrillada_split_dialog_size_error (self))
			return FALSE;

		if (warn
		&&  end - (pos + 1) < PARRILLADA_MIN_STREAM_LENGTH
		&& !parrillada_split_dialog_size_error (self))
			return FALSE;

		length_str = parrillada_units_get_time_string (pos - priv->start, TRUE, FALSE);
		start_str = parrillada_units_get_time_string (priv->start, TRUE, FALSE);
		end_str = parrillada_units_get_time_string (pos, TRUE, FALSE);
		...
		pos ++;
		length_str = parrillada_units_get_time_string (end - pos, TRUE, FALSE);
		start_str = parrillada_units_get_time_string (pos, TRUE, FALSE);
		end_str = parrillada_units_get_time_string (end, TRUE, FALSE);
	...
	do {
		...
		if (warn
		&& (pos - start) < PARRILLADA_MIN_STREAM_LENGTH
		&& !parrillada_split_dialog_size_error (self))
			return FALSE;
	...
	if (slice.start == 0 && slice.end == 0) {
		...
		if (warn
		&& (slice.end - slice.start) < PARRILLADA_MIN_STREAM_LENGTH
		&& !parrillada_split_dialog_size_error (self))
			return FALSE;
	}
	else {
		/* we are in the middle of an existing slice */
		length_str = parrillada_units_get_time_string (pos - slice.start, TRUE, FALSE);
		end_str = parrillada_units_get_time_string (pos, TRUE, FALSE);
	...
	length_str = parrillada_units_get_time_string (slice.end - pos, TRUE, FALSE);
	start_str = parrillada_units_get_time_string (pos, TRUE, FALSE);
	end_str = parrillada_units_get_time_string (slice.end, TRUE, FALSE);
...
static void
parrillada_split_dialog_remove_range (ParrilladaSplitDialog *self,
				   gint64 start,
				   gint64 end,
				   gint64 length)
{
	...
	if (!gtk_tree_model_get_iter_first (model, &iter)) {
		...
		if (start - priv->start < PARRILLADA_MIN_STREAM_LENGTH
		&& !parrillada_split_dialog_size_error (self)) {
			/* that's not necessarily a good solution */
			start = PARRILLADA_MIN_STREAM_LENGTH;
			if (start > end)
				end = start;
		}

		if ((length - end) < PARRILLADA_MIN_STREAM_LENGTH
		&& !parrillada_split_dialog_size_error (self))
			end = length - PARRILLADA_MIN_STREAM_LENGTH;

		length_str = parrillada_units_get_time_string (start - priv->start, TRUE, FALSE);
		start_str = parrillada_units_get_time_string (priv->start, TRUE, FALSE);
		end_str = parrillada_units_get_time_string (start, TRUE, FALSE);
		...
		length_str = parrillada_units_get_time_string (length - end, TRUE, FALSE);
		start_str = parrillada_units_get_time_string (end, TRUE, FALSE);
		end_str = parrillada_units_get_time_string (length, TRUE, FALSE);
	...
	do {
		...
		if (start == track_start) {
			...
			if (end < track_end) {
				 ...
				 * track is > 4 sec */
				if ((track_end - end) < PARRILLADA_MIN_STREAM_LENGTH
				&& !parrillada_split_dialog_size_error (self))
					end = track_end - PARRILLADA_MIN_STREAM_LENGTH;

				start_str = parrillada_units_get_time_string (end, TRUE, FALSE);
				length_str = parrillada_units_get_time_string (track_end - end, TRUE, FALSE);
				gtk_list_store_set (priv->model, &child,
						    START_COL, end,
						    START_STR_COL, start_str,
			...
			if ((start - track_start) < PARRILLADA_MIN_STREAM_LENGTH
			&& !parrillada_split_dialog_size_error (self))
				start = track_start + PARRILLADA_MIN_STREAM_LENGTH;

			start_str = parrillada_units_get_time_string (start, TRUE, FALSE);
			length_str = parrillada_units_get_time_string (start - track_start, TRUE, FALSE);
			gtk_list_store_set (priv->model, &child,
					    END_COL, start,
			...
			length_str = parrillada_units_get_time_string (track_end - end, TRUE, FALSE);
			start_str = parrillada_units_get_time_string (end, TRUE, FALSE);
			end_str = parrillada_units_get_time_string (track_end, TRUE, FALSE);
...
static void
parrillada_split_dialog_no_silence_message (ParrilladaSplitDialog *self)
{
...
static void
parrillada_split_dialog_metadata_finished_cb (ParrilladaMetadata *metadata,
					   GError *error,
					   ParrilladaSplitDialog *self)
{
	ParrilladaMetadataInfo info = { NULL, };
	ParrilladaSplitDialogPrivate *priv;
	...
	parrillada_metadata_get_result (metadata, &info, NULL);
	if (!info.silences) {
		parrillada_split_dialog_no_silence_message (self);
		return;
	}
	...
	for (iter = info.silences; iter; iter = iter->next) {
		...
		if (!silence->start)
			parrillada_split_dialog_cut (self, silence->end, TRUE);
		else if (silence->start != silence->end)
			parrillada_split_dialog_remove_range (self,
							   silence->start,
							   silence->end,
							   priv->end - priv->start);
		else
...
static gboolean
parrillada_split_dialog_clear_confirm_dialog (ParrilladaSplitDialog *self,
					   const gchar *primary,
...
static void
parrillada_split_dialog_cut_clicked_cb (GtkButton *button,
				     ParrilladaSplitDialog *self)
{
	ParrilladaSplitDialogPrivate *priv;
	guint page;
	...
	if (page == 0) {
		...
		pos = parrillada_song_control_get_pos (PARRILLADA_SONG_CONTROL (priv->player));
		parrillada_split_dialog_cut (self, pos + priv->start, TRUE);
		return;
	}
	...
	if (page == 1) {
		...
		sec *= 1000000000;
		if (sec < PARRILLADA_MIN_STREAM_LENGTH
		&& !parrillada_split_dialog_size_error (self))
			return;
		...
		for (start = sec; start < length; start += sec)
			parrillada_split_dialog_cut (self, start, FALSE);
	...
	if (page == 2) {
		...
		if (step < PARRILLADA_MIN_STREAM_LENGTH
		&& !parrillada_split_dialog_size_error (self))
			return;
	...
	priv->metadata = parrillada_metadata_new ();
	g_signal_connect (priv->metadata,
			  "completed",
			  G_CALLBACK (parrillada_split_dialog_metadata_finished_cb),
			  self);
	parrillada_metadata_get_info_async (priv->metadata,
					 parrillada_song_control_get_uri (PARRILLADA_SONG_CONTROL (priv->player)),
					 PARRILLADA_METADATA_FLAG_SILENCES);
...
static void
parrillada_split_dialog_merge_clicked_cb (GtkButton *button,
				       ParrilladaSplitDialog *self)
{
	ParrilladaSplitDialogPrivate *priv;
	...
	do {
		...
		length_str = parrillada_units_get_time_string (end - start, TRUE, FALSE);
		start_str = parrillada_units_get_time_string (start, TRUE, FALSE);
		end_str = parrillada_units_get_time_string (end, TRUE, FALSE);
...
static void
parrillada_split_dialog_remove_clicked_cb (GtkButton *button,
				        ParrilladaSplitDialog *self)
{
	ParrilladaSplitDialogPrivate *priv;
...
static void
parrillada_split_dialog_reset_clicked_cb (GtkButton *button,
				       ParrilladaSplitDialog *self)
{
	ParrilladaSplitDialogPrivate *priv;
...
static void
parrillada_split_dialog_combo_changed_cb (GtkComboBox *combo,
				       ParrilladaSplitDialog *self)
{
	ParrilladaSplitDialogPrivate *priv;
	guint page;
...
static void
parrillada_split_dialog_selection_changed_cb (GtkTreeSelection *selection,
					   ParrilladaSplitDialog *self)
{
	ParrilladaSplitDialogPrivate *priv;
	GList *selected;
...
static void
parrillada_split_dialog_row_inserted_cb (GtkTreeModel *model,
				      GtkTreePath *path,
				      GtkTreeIter *iter,
				      ParrilladaSplitDialog *self)
{
...
static void
parrillada_split_dialog_row_deleted_cb (GtkTreeModel *model,
				     GtkTreePath *path,
				     ParrilladaSplitDialog *self)
{
	ParrilladaSplitDialogPrivate *priv;
...
static void
parrillada_split_dialog_init (ParrilladaSplitDialog *object)
{
	gchar *title;
	GtkWidget *vbox;
	GtkWidget *hbox;
	...
	priv = PARRILLADA_SPLIT_DIALOG_PRIVATE (object);
	...
	button = parrillada_utils_make_button (_("_Slice"),
					    NULL,
	...
	g_signal_connect (button,
			  "clicked",
			  G_CALLBACK (parrillada_split_dialog_cut_clicked_cb),
			  object);
	...
	priv->player = parrillada_song_control_new ();
	...
	gtk_box_pack_start (GTK_BOX (vbox),
			    parrillada_utils_pack_properties (title,
							   priv->notebook,
							   hbox,
							   NULL),
			    FALSE,
	...
	g_signal_connect (priv->model,
			  "row-inserted",
			  G_CALLBACK (parrillada_split_dialog_row_inserted_cb),
			  object);
	g_signal_connect (priv->model,
			  "row-deleted",
			  G_CALLBACK (parrillada_split_dialog_row_deleted_cb),
			  object);
	...
	g_signal_connect (gtk_tree_view_get_selection (GTK_TREE_VIEW (priv->tree)),
			  "changed",
			  G_CALLBACK (parrillada_split_dialog_selection_changed_cb),
			  object);
	...
	button = parrillada_utils_make_button (_("Mer_ge"),
					    NULL,
					    NULL,
					    GTK_ICON_SIZE_BUTTON);
	gtk_widget_show (button);
	...
	g_signal_connect (button,
			  "clicked",
			  G_CALLBACK (parrillada_split_dialog_remove_clicked_cb),
			  object);
	gtk_widget_set_tooltip_text (button, _("Remove the selected slices"));
	priv->remove_button = button;

	button = parrillada_utils_make_button (_("Re_move All"),
					    NULL,
					    NULL,
					    GTK_ICON_SIZE_BUTTON);
	gtk_widget_show (button);
	...
	g_signal_connect (button,
			  "clicked",
			  G_CALLBACK (parrillada_split_dialog_reset_clicked_cb),
			  object);
...
static void
parrillada_split_dialog_finalize (GObject *object)
{
	ParrilladaSplitDialogPrivate *priv;

	priv = PARRILLADA_SPLIT_DIALOG_PRIVATE (object);
	if (priv->metadata) {
		parrillada_metadata_cancel (priv->metadata);
		g_object_unref (priv->metadata);
		priv->metadata = NULL;
	}

	G_OBJECT_CLASS (parrillada_split_dialog_parent_class)->finalize (object);
}

static void
parrillada_split_dialog_class_init (ParrilladaSplitDialogClass *klass)
{
	GObjectClass* object_class = G_OBJECT_CLASS (klass);

	g_type_class_add_private (klass, sizeof (ParrilladaSplitDialogPrivate));

	object_class->finalize = parrillada_split_dialog_finalize;
}

GtkWidget *
parrillada_split_dialog_new (void)
{
	return GTK_WIDGET (g_object_new (PARRILLADA_TYPE_SPLIT_DIALOG, NULL));
}
