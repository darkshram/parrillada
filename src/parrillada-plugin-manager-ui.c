...
typedef enum {
	PARRILLADA_PLUGIN_BURN_ENGINE			= 0,
	PARRILLADA_PLUGIN_IMAGE_ENGINE,
	PARRILLADA_PLUGIN_CONVERT_ENGINE,
	PARRILLADA_PLUGIN_MISCELLANEOUS,
	PARRILLADA_PLUGIN_ERROR
} ParrilladaPluginCategory;
...
#define PARRILLADA_PLUGIN_MANAGER_UI_GET_PRIVATE(object)(G_TYPE_INSTANCE_GET_PRIVATE ((object), PARRILLADA_TYPE_PLUGIN_MANAGER_UI, ParrilladaPluginManagerUIPrivate))

struct _ParrilladaPluginManagerUIPrivate
{
	GtkWidget	*tree;
...
static void 
parrillada_plugin_manager_ui_class_init (ParrilladaPluginManagerUIClass *klass)
{
	GObjectClass *object_class = G_OBJECT_CLASS (klass);

	object_class->finalize = parrillada_plugin_manager_ui_finalize;

	g_type_class_add_private (object_class, sizeof (ParrilladaPluginManagerUIPrivate));
}

static ParrilladaPlugin *
plugin_manager_ui_get_selected_plugin (ParrilladaPluginManagerUI *pm)
{
	ParrilladaPlugin *plugin = NULL;
	GtkTreeModel *model;
	GtkTreeIter iter;
	GtkTreeSelection *selection;
	ParrilladaPluginManagerUIPrivate *priv;
		 ...
		 ParrilladaPluginManagerUI *pm)
{
	...
	authors [0] = parrillada_plugin_get_author (plugin);

	copyright = g_strdup_printf (_("Copyright %s"),
				     parrillada_plugin_get_author (plugin));

	dialog = g_object_new (GTK_TYPE_ABOUT_DIALOG,
			       "program-name", _(parrillada_plugin_get_display_name (plugin)),
			       "copyright", copyright,
			       "authors", authors,
			       "comments", parrillada_plugin_get_description (plugin),
			       "logo-icon-name", parrillada_plugin_get_icon_name (plugin),
			       NULL);
		     ...
		     ParrilladaPluginManagerUI *pm)
{
	...
	parrillada_plugin_option_set_plugin (PARRILLADA_PLUGIN_OPTION (dialog), plugin);
	gtk_window_set_transient_for (GTK_WINDOW (dialog),
				      GTK_WINDOW (gtk_widget_get_toplevel (GTK_WIDGET (pm))));
				     ...
				     gpointer           data)
{
	...
	if (error_string) {
		text = g_markup_printf_escaped ("<b>%s</b>\n%s\n<i>%s</i>",
						 ...
						 * the plugin. */
						_(parrillada_plugin_get_display_name (plugin)),
						parrillada_plugin_get_description (plugin),
						error_string);
		g_free (error_string);
	}
	else
...
{
	ParrilladaPlugin *plugin;
	ParrilladaPluginManagerUI *pm = data;
	ParrilladaPluginManagerUIPrivate *priv;
	...
	gtk_widget_set_sensitive (GTK_WIDGET (priv->configure_button),
				  (plugin != NULL) && 
				   parrillada_plugin_get_next_conf_option (plugin, NULL));
}
...
static void
plugin_manager_ui_populate_lists (ParrilladaPluginManagerUI *pm)
{
	ParrilladaPluginManagerUIPrivate *priv;
	ParrilladaPlugin *plugin;
	...
	for (; plugins; plugins = plugins->next) {
		...
		gtk_list_store_set (model, &iter,
				    ACTIVE_COLUMN, parrillada_plugin_get_active (plugin, 0),
				    AVAILABLE_COLUMN, parrillada_plugin_get_gtype (plugin) != G_TYPE_NONE && !parrillada_plugin_get_compulsory (plugin),
				    PLUGIN_COLUMN, plugin,
				    -1);
	}
	...
	if (gtk_tree_model_get_iter_first (GTK_TREE_MODEL (model), &iter))
	{
		...
		gtk_widget_set_sensitive (GTK_WIDGET (priv->configure_button),
					  (parrillada_plugin_get_next_conf_option (plugin, NULL) != NULL));
	}
			      ...
			      gboolean      active)
{
	...
	||  parrillada_plugin_get_compulsory (plugin))
		return FALSE;
...
static void
plugin_manager_ui_set_active_all (ParrilladaPluginManagerUI *pm,
				  gboolean            active)
{
	ParrilladaPluginManagerUIPrivate *priv;
	GtkTreeModel *model;
		...
		gpointer      data)
{
	...
	normalized_string = g_utf8_normalize (_(parrillada_plugin_get_display_name (plugin)),
					      -1,
					      G_NORMALIZE_ALL);
...
static GtkWidget *
create_tree_popup_menu (ParrilladaPluginManagerUI *pm)
{
	GtkWidget *menu;
	GtkWidget *item;
	GtkWidget *image;
	...
	gtk_check_menu_item_set_active (GTK_CHECK_MENU_ITEM (item),
					parrillada_plugin_get_active (plugin, 0));
...
static void
tree_popup_menu_detach (ParrilladaPluginManagerUI *pm,
			GtkMenu            *menu)
{
	ParrilladaPluginManagerUIPrivate *priv;
...
model_name_sort_func (GtkTreeModel *model,
		      ...
		      gpointer      user_data)
{
	ParrilladaPlugin *plugin1, *plugin2;
	...
	active1 = parrillada_plugin_get_gtype (plugin1) != G_TYPE_NONE && !parrillada_plugin_get_compulsory (plugin1);
	active2 = parrillada_plugin_get_gtype (plugin2) != G_TYPE_NONE && !parrillada_plugin_get_compulsory (plugin2);
	...
	return g_utf8_collate (_(parrillada_plugin_get_display_name (plugin1)),
			       _(parrillada_plugin_get_display_name (plugin2)));
}

static void
plugin_manager_ui_construct_tree (ParrilladaPluginManagerUI *pm)
{
	ParrilladaPluginManagerUIPrivate *priv;
	GtkTreeViewColumn *column;
	GtkCellRenderer *cell;
...
static void 
parrillada_plugin_manager_ui_init (ParrilladaPluginManagerUI *pm)
{
	GtkWidget *vbox;
	GtkWidget *hbox;
	GtkWidget *viewport;
	...
	priv->about_button = parrillada_utils_make_button (_("_About"),
							NULL,
							GTK_STOCK_ABOUT,
							GTK_ICON_SIZE_BUTTON);
	gtk_container_add (GTK_CONTAINER (vbuttonbox), priv->about_button);

	priv->configure_button = parrillada_utils_make_button (_("C_onfigure"),
							    NULL,
	...
	priv->plugins = parrillada_burn_library_get_plugins_list ();
...
static void
parrillada_plugin_manager_ui_finalize (GObject *object)
{
	ParrilladaPluginManagerUI *pm = PARRILLADA_PLUGIN_MANAGER_UI (object);
	ParrilladaPluginManagerUIPrivate *priv;
	...
	G_OBJECT_CLASS (parrillada_plugin_manager_ui_parent_class)->finalize (object);

}

GtkWidget *parrillada_plugin_manager_ui_new (void)
{
	return g_object_new (PARRILLADA_TYPE_PLUGIN_MANAGER_UI,0);
}
