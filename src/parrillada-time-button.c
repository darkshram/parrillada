...
typedef struct _ParrilladaTimeButtonPrivate ParrilladaTimeButtonPrivate;
struct _ParrilladaTimeButtonPrivate
{
	GtkWidget *hrs;
	GtkWidget *min;
	GtkWidget *sec;
...
#define PARRILLADA_TIME_BUTTON_PRIVATE(o)  (G_TYPE_INSTANCE_GET_PRIVATE ((o), PARRILLADA_TYPE_TIME_BUTTON, ParrilladaTimeButtonPrivate))
...
G_DEFINE_TYPE (ParrilladaTimeButton, parrillada_time_button, GTK_TYPE_HBOX);

static void
parrillada_time_button_hrs_changed (GtkSpinButton *button,
				 ParrilladaTimeButton *self)
{
	ParrilladaTimeButtonPrivate *priv;
	gint hrs, min;

	priv = PARRILLADA_TIME_BUTTON_PRIVATE (self);
...
static void
parrillada_time_button_min_changed (GtkSpinButton *button,
				 ParrilladaTimeButton *self)
{
	ParrilladaTimeButtonPrivate *priv;
	gint hrs, min, sec;
...
static void
parrillada_time_button_sec_changed (GtkSpinButton *button,
				 ParrilladaTimeButton *self)
{
	ParrilladaTimeButtonPrivate *priv;
...
static void
parrillada_time_button_frame_changed (GtkSpinButton *button,
				   ParrilladaTimeButton *self)
{
	g_signal_emit (self,
		       time_button_signals [VALUE_CHANGED_SIGNAL],
...
void
parrillada_time_button_set_show_frames (ParrilladaTimeButton *self,
				     gboolean show)
{
	ParrilladaTimeButtonPrivate *priv;
...
void
parrillada_time_button_set_max (ParrilladaTimeButton *self,
			     gint64 max)
{
	ParrilladaTimeButtonPrivate *priv;
	gint64 frames;
...
void
parrillada_time_button_set_value (ParrilladaTimeButton *self,
			       gint64 value)
{
	ParrilladaTimeButtonPrivate *priv;
	gint64 frames;
...
gint64
parrillada_time_button_get_value (ParrilladaTimeButton *self)
{
	ParrilladaTimeButtonPrivate *priv;
	gint64 value;
...
static void
parrillada_time_button_init (ParrilladaTimeButton *object)
{
	ParrilladaTimeButtonPrivate *priv;
	GtkWidget *label;

	priv = PARRILLADA_TIME_BUTTON_PRIVATE (object);
	...
	g_signal_connect (priv->hrs,
			  "value-changed",
			  G_CALLBACK (parrillada_time_button_hrs_changed),
			  object);
	g_signal_connect (priv->min,
			  "value-changed",
			  G_CALLBACK (parrillada_time_button_min_changed),
			  object);
	g_signal_connect (priv->sec,
			  "value-changed",
			  G_CALLBACK (parrillada_time_button_sec_changed),
			  object);
	g_signal_connect (priv->frame,
			  "value-changed",
			  G_CALLBACK (parrillada_time_button_frame_changed),
			  object);
}

static void
parrillada_time_button_finalize (GObject *object)
{
	G_OBJECT_CLASS (parrillada_time_button_parent_class)->finalize (object);
}

static void
parrillada_time_button_class_init (ParrilladaTimeButtonClass *klass)
{
	GObjectClass* object_class = G_OBJECT_CLASS (klass);

	g_type_class_add_private (klass, sizeof (ParrilladaTimeButtonPrivate));

	object_class->finalize = parrillada_time_button_finalize;

	time_button_signals[VALUE_CHANGED_SIGNAL] =
		g_signal_new ("value-changed",
		              G_OBJECT_CLASS_TYPE (klass),
		              G_SIGNAL_RUN_FIRST | G_SIGNAL_NO_RECURSE,
		              G_STRUCT_OFFSET (ParrilladaTimeButtonClass, value_changed),
		              NULL, NULL,
...
{
	return g_object_new (PARRILLADA_TYPE_TIME_BUTTON, NULL);
}
