...
#include "parrillada-split-dialog.h"
#include "parrillada-video-tree-model.h"

#include "eggtreemultidnd.h"

static ParrilladaDiscResult
parrillada_audio_disc_set_session_contents (ParrilladaDisc *disc,
					 ParrilladaBurnSession *session);

static ParrilladaDiscResult
parrillada_audio_disc_add_uri (ParrilladaDisc *disc,
			    const char *uri);
static void
parrillada_audio_disc_delete_selected (ParrilladaDisc *disc);


static guint
parrillada_audio_disc_add_ui (ParrilladaDisc *disc,
			   GtkUIManager *manager,
			   GtkWidget *message);

static gboolean
parrillada_audio_disc_button_pressed_cb (GtkTreeView *tree,
				      GdkEventButton *event,
				      ParrilladaAudioDisc *disc);
static gboolean
parrillada_audio_disc_key_released_cb (GtkTreeView *tree,
				    GdkEventKey *event,
				    ParrilladaAudioDisc *disc);
static void
parrillada_audio_disc_display_edited_cb (GtkCellRendererText *cellrenderertext,
				      gchar *path_string,
				      gchar *text,
				      ParrilladaAudioDisc *disc);
static void
parrillada_audio_disc_display_editing_started_cb (GtkCellRenderer *renderer,
					       GtkCellEditable *editable,
					       gchar *path,
					       ParrilladaAudioDisc *disc);
static void
parrillada_audio_disc_display_editing_canceled_cb (GtkCellRenderer *renderer,
						 ParrilladaAudioDisc *disc);

static void
parrillada_audio_disc_edit_information_cb (GtkAction *action,
					ParrilladaAudioDisc *disc);
static void
parrillada_audio_disc_open_activated_cb (GtkAction *action,
				      ParrilladaAudioDisc *disc);
static void
parrillada_audio_disc_delete_activated_cb (GtkAction *action,
					ParrilladaDisc *disc);
static void
parrillada_audio_disc_paste_activated_cb (GtkAction *action,
				       ParrilladaAudioDisc *disc);
static gboolean
parrillada_audio_disc_get_selected_uri (ParrilladaDisc *disc,
				     gchar **uri);
static gboolean
parrillada_audio_disc_get_boundaries (ParrilladaDisc *disc,
				   gint64 *start,
				   gint64 *end);

static void
parrillada_audio_disc_add_pause_cb (GtkAction *action, ParrilladaAudioDisc *disc);
static void
parrillada_audio_disc_split_cb (GtkAction *action, ParrilladaAudioDisc *disc);

static void
parrillada_audio_disc_selection_changed (GtkTreeSelection *selection, ParrilladaAudioDisc *disc);

struct _ParrilladaAudioDiscPrivate {
	ParrilladaIOJobBase *add_dir;
	ParrilladaIOJobBase *add_playlist;
...
#define PARRILLADA_AUDIO_DISC_CONTEXT		1000

static void parrillada_audio_disc_iface_disc_init (ParrilladaDiscIface *iface);
...
static gboolean
parrillada_audio_disc_is_empty (ParrilladaDisc *disc)
{
	GtkTreeModel *model;

	model = gtk_tree_view_get_model (GTK_TREE_VIEW (PARRILLADA_AUDIO_DISC (disc)->priv->tree));
	if (!model)
		return FALSE;
...
static void
parrillada_audio_disc_iface_disc_init (ParrilladaDiscIface *iface)
{
	iface->add_uri = parrillada_audio_disc_add_uri;
	iface->delete_selected = parrillada_audio_disc_delete_selected;

	iface->is_empty = parrillada_audio_disc_is_empty;

	iface->set_session_contents = parrillada_audio_disc_set_session_contents;
	iface->get_selected_uri = parrillada_audio_disc_get_selected_uri;
	iface->get_boundaries = parrillada_audio_disc_get_boundaries;
	iface->add_ui = parrillada_audio_disc_add_ui;
}

static void
parrillada_audio_disc_get_property (GObject * object,
				 guint prop_id,
				 GValue * value,
				 GParamSpec * pspec)
{
	...
	disc = PARRILLADA_AUDIO_DISC (object);
...
static void
parrillada_audio_disc_set_property (GObject * object,
				 guint prop_id,
				 const GValue * value,
				 GParamSpec * pspec)
{
...
parrillada_audio_disc_add_ui (ParrilladaDisc *disc,
			   ...
			   GtkWidget *message)
{
	ParrilladaAudioDisc *audio_disc;
	GError *error = NULL;
	GtkAction *action;
	guint merge_id;

	audio_disc = PARRILLADA_AUDIO_DISC (disc);
	...
	if (!audio_disc->priv->disc_group) {
		audio_disc->priv->disc_group = gtk_action_group_new (PARRILLADA_DISC_ACTION "-audio");
...
static gboolean
parrillada_audio_disc_selection_function (GtkTreeSelection *selection,
				       GtkTreeModel *model,
...
static void
parrillada_audio_disc_init (ParrilladaAudioDisc *obj)
{
	...
	g_signal_connect (gtk_tree_view_get_selection (GTK_TREE_VIEW (obj->priv->tree)),
			  "changed",
			  G_CALLBACK (parrillada_audio_disc_selection_changed),
			  obj);
	...
	g_signal_connect (G_OBJECT (obj->priv->tree),
			  "button-press-event",
			  G_CALLBACK (parrillada_audio_disc_button_pressed_cb),
			  obj);
	g_signal_connect (G_OBJECT (obj->priv->tree),
			  "key-release-event",
			  G_CALLBACK (parrillada_audio_disc_key_released_cb),
			  obj);

	selection = gtk_tree_view_get_selection (GTK_TREE_VIEW (obj->priv->tree));
	gtk_tree_selection_set_mode (selection,GTK_SELECTION_MULTIPLE);
	gtk_tree_selection_set_select_function (selection, parrillada_audio_disc_selection_function, NULL, NULL);
	...
	column = gtk_tree_view_column_new_with_attributes (_("Track"), renderer,
							   "text", PARRILLADA_VIDEO_TREE_MODEL_INDEX,
							   NULL);
	...
	gtk_tree_view_column_add_attribute (column, renderer,
					    "icon-name", PARRILLADA_VIDEO_TREE_MODEL_ICON_NAME);

	renderer = gtk_cell_renderer_text_new ();
	g_object_set_data (G_OBJECT (renderer), COL_KEY, PARRILLADA_TRACK_STREAM_TITLE_TAG);
	...
	g_signal_connect (G_OBJECT (renderer), "edited",
			  G_CALLBACK (parrillada_audio_disc_display_edited_cb), obj);
	g_signal_connect (G_OBJECT (renderer), "editing-started",
			  G_CALLBACK (parrillada_audio_disc_display_editing_started_cb), obj);
	g_signal_connect (G_OBJECT (renderer), "editing-canceled",
			  G_CALLBACK (parrillada_audio_disc_display_editing_canceled_cb), obj);

	gtk_tree_view_column_pack_end (column, renderer, TRUE);
	gtk_tree_view_column_add_attribute (column, renderer,
					    "text", PARRILLADA_VIDEO_TREE_MODEL_NAME);
	gtk_tree_view_column_add_attribute (column, renderer,
					    "style", PARRILLADA_VIDEO_TREE_MODEL_STYLE);
	gtk_tree_view_column_add_attribute (column, renderer,
					    "weight", PARRILLADA_VIDEO_TREE_MODEL_WEIGHT);
	gtk_tree_view_column_add_attribute (column, renderer,
	...
	renderer = gtk_cell_renderer_text_new ();
	g_object_set_data (G_OBJECT (renderer), COL_KEY, PARRILLADA_TRACK_STREAM_ARTIST_TAG);
	g_object_set (G_OBJECT (renderer),
	...
	column = gtk_tree_view_column_new_with_attributes (_("Artist"), renderer,
							   "text", PARRILLADA_VIDEO_TREE_MODEL_ARTIST,
							   NULL);
	gtk_tree_view_append_column (GTK_TREE_VIEW (obj->priv->tree), column);
	gtk_tree_view_column_set_resizable (column, TRUE);
...
static void
parrillada_audio_disc_reset_real (ParrilladaAudioDisc *disc)
{
	parrillada_io_cancel_by_base (disc->priv->add_dir);
	parrillada_io_cancel_by_base (disc->priv->add_playlist);
	...
	if (disc->priv->message)
		parrillada_notify_message_remove (disc->priv->message, PARRILLADA_AUDIO_DISC_CONTEXT);
}

static void
parrillada_audio_disc_finalize (GObject *object)
{
	ParrilladaAudioDisc *cobj;
	cobj = PARRILLADA_AUDIO_DISC(object);
	
	parrillada_audio_disc_reset_real (cobj);

	parrillada_io_job_base_free (cobj->priv->add_dir);
	parrillada_io_job_base_free (cobj->priv->add_playlist);
...
static void
parrillada_audio_disc_add_gap (ParrilladaAudioDisc *disc,
			    GtkTreePath *treepath,
			    gint64 gap)
{
	GtkTreeModel *model;
...
static void
parrillada_audio_disc_short_track_dialog (ParrilladaAudioDisc *disc)
{
...
static ParrilladaDiscResult
parrillada_audio_disc_add_uri_real (ParrilladaAudioDisc *disc,
				 const gchar *uri,
				 gint pos,
				 gint64 gap_sectors,
				 gint64 start,
				 ...
				 GtkTreePath **path_return)
{
	...
	session = parrillada_video_tree_model_get_session (PARRILLADA_VIDEO_TREE_MODEL (store));
	...
	parrillada_track_stream_set_boundaries (PARRILLADA_TRACK_STREAM (track),
					     ...
					     PARRILLADA_SECTORS_TO_DURATION (gap_sectors));
	...
	if (path_return)
		*path_return = parrillada_video_tree_model_track_to_path (PARRILLADA_VIDEO_TREE_MODEL (store), PARRILLADA_TRACK (track));
...
static void
parrillada_audio_disc_file_type_error_dialog (ParrilladaAudioDisc *disc,
					   const gchar *uri)
{
	gchar *primary;
	gchar *name;
...
static gboolean
parrillada_audio_disc_video_file_dialog (ParrilladaAudioDisc *disc,
				      const gchar *uri)
{
	GtkWidget *dialog;
...
static void
parrillada_audio_disc_result (GObject *obj,
			   GError *error,
			   const gchar *uri,
			   GFileInfo *info,
			   gpointer null_data)
{
	ParrilladaAudioDisc *disc = PARRILLADA_AUDIO_DISC (obj);

	if (error)
		return;

	/* we silently ignore the title and any error */
	if (g_file_info_get_attribute_string (info, PARRILLADA_IO_PLAYLIST_TITLE))
		return;

	if (!g_file_info_get_attribute_boolean (info, PARRILLADA_IO_HAS_AUDIO))
		return;

	if (g_file_info_get_attribute_boolean (info, PARRILLADA_IO_HAS_VIDEO)
	&& !parrillada_audio_disc_video_file_dialog (disc, uri))
		return;

	parrillada_audio_disc_add_uri_real (disc,
					 uri,
					 -1,
					 -1,
					 -1,
...
static ParrilladaDiscResult
parrillada_audio_disc_add_directory_contents (ParrilladaAudioDisc *disc,
					   const gchar *uri)
{
	if (!disc->priv->add_dir)
		disc->priv->add_dir = parrillada_io_register (G_OBJECT (disc),
							   parrillada_audio_disc_result,
							   NULL,
							   NULL);
...
static gboolean
parrillada_audio_disc_add_dir (ParrilladaAudioDisc *disc, const gchar *uri)
{
	gint answer;
	GtkWidget *dialog;
...
static ParrilladaDiscResult
parrillada_audio_disc_add_playlist (ParrilladaAudioDisc *disc,
				 const gchar *uri)
{
	if (!disc->priv->add_playlist)
		disc->priv->add_playlist = parrillada_io_register (G_OBJECT (disc),
								parrillada_audio_disc_result,
								NULL,
								NULL);
...
/**************************** New Row ******************************************/
static void
parrillada_audio_disc_unreadable_dialog (ParrilladaAudioDisc *disc,
				      const gchar *uri,
				      GError *error)
{
	gchar *primary;
...
static void
parrillada_audio_disc_wav_dts_response_cb (GtkButton *button,
                                        ...
                                        ParrilladaAudioDisc *disc)
{
	...
	if (response == GTK_RESPONSE_OK)
		parrillada_burn_session_tag_add_int (PARRILLADA_BURN_SESSION (session),
		                                  PARRILLADA_SESSION_STREAM_AUDIO_FORMAT,
		                                  PARRILLADA_AUDIO_FORMAT_DTS);
}

static void
parrillada_audio_disc_wav_dts_file_dialog (ParrilladaAudioDisc *disc)
{
	GtkWidget *message;
	ParrilladaSessionCfg *session;
	GtkTreeModel *model;

	if (parrillada_notify_get_message_by_context_id (disc->priv->message,
	                                              PARRILLADA_AUDIO_DISC_CONTEXT))
		return;
	...
	message = parrillada_notify_message_add (disc->priv->message,
	...
	g_signal_connect (PARRILLADA_DISC_MESSAGE (message),
			  "response",
			  G_CALLBACK (parrillada_audio_disc_wav_dts_response_cb),
			  disc);
}

static void
parrillada_audio_disc_session_changed (ParrilladaSessionCfg *session,
				    ParrilladaAudioDisc *self)
{
	GSList *next;
	GSList *tracks;
	gboolean notready;
	...
	for (; tracks; tracks = next) {
		...
		if (result == PARRILLADA_BURN_ERR) {
			...
			if (!error)
				parrillada_audio_disc_file_type_error_dialog (self, uri);
			else if (error->code == PARRILLADA_BURN_ERROR_FILE_FOLDER) {
				res = parrillada_audio_disc_add_dir (self, uri);
				if (res)
					parrillada_audio_disc_add_directory_contents (self, uri);
			}

#if BUILD_PLAYLIST
			else if (error->code == PARRILLADA_BURN_ERROR_FILE_PLAYLIST) {
				/* This is a supported playlist */
				parrillada_audio_disc_add_playlist (self, uri);
			}
#endif
				...
				parrillada_audio_disc_unreadable_dialog (self,
								      uri,
								      error);
		...
		if (!PARRILLADA_STREAM_FORMAT_AUDIO (format)) {
			gchar *uri;
		...
		if ((format & PARRILLADA_AUDIO_FORMAT_DTS) != 0)
			should_use_dts = TRUE;
		...
		if (PARRILLADA_STREAM_FORMAT_HAS_VIDEO (format)) {
			...
			uri = parrillada_track_stream_get_source (track, TRUE);
			res = parrillada_audio_disc_video_file_dialog (self, uri);
			if (res)
				parrillada_burn_session_remove_track (PARRILLADA_BURN_SESSION (session),
								   PARRILLADA_TRACK (track));
			g_free (uri);
	...
	if (should_use_dts
	&&  parrillada_burn_session_tag_lookup (PARRILLADA_BURN_SESSION (session), PARRILLADA_SESSION_STREAM_AUDIO_FORMAT, NULL) != PARRILLADA_BURN_OK)
		parrillada_audio_disc_wav_dts_file_dialog (self);
}
			    ...
			    const gchar *uri)
{
	...
	audio_disc = PARRILLADA_AUDIO_DISC (disc);
	result = parrillada_audio_disc_add_uri_real (audio_disc,
						  uri,
						  -1,
						  0,
						  -1,
...
static void
parrillada_audio_disc_remove (ParrilladaAudioDisc *disc,
			   GtkTreePath *treepath)
{
	ParrilladaSessionCfg *session;
	GtkTreeModel *model;
	...
	gtk_tree_model_get (model, &iter,
			    PARRILLADA_VIDEO_TREE_MODEL_IS_GAP, &is_gap,
			    -1);
...
{
	GtkTreeSelection *selection;
	ParrilladaAudioDisc *audio;
	GtkTreePath *treepath;
	GtkTreeModel *model;
	GList *list;

	audio = PARRILLADA_AUDIO_DISC (disc);
	...
	for (; list; list = g_list_remove (list, treepath)) {
		treepath = list->data;
		parrillada_audio_disc_remove (audio, treepath);
		gtk_tree_path_free (treepath);
	}
					 ...
					 ParrilladaBurnSession *session)
{
	...
	if (audio->priv->add_dir)
		parrillada_io_cancel_by_base (audio->priv->add_dir);

	if (audio->priv->add_playlist)
		parrillada_io_cancel_by_base (audio->priv->add_playlist);
	...
	if (current_model) {
		...
		if (current_session)
			g_signal_handlers_disconnect_by_func (current_session,
							      parrillada_audio_disc_session_changed,
							      disc);
	}
	...
	g_signal_connect (session,
			  "is-valid",
			  G_CALLBACK (parrillada_audio_disc_session_changed),
			  disc);
...
/********************************** Cell Editing *******************************/
static void
parrillada_audio_disc_display_edited_cb (GtkCellRendererText *renderer,
				      gchar *path_string,
				      gchar *text,
				      ParrilladaAudioDisc *disc)
{
...
/********************************** Pause **************************************/
static void
parrillada_audio_disc_add_pause (ParrilladaAudioDisc *disc)
{
	GtkTreeSelection *selection;
	GtkTreePath *treepath;
	GtkTreeModel *model;
	...
	for (iter = references; iter; iter = iter->next) {
		...
		parrillada_audio_disc_add_gap (disc, treepath, 2 * GST_SECOND);
		gtk_tree_path_free (treepath);
	}
...
static void
parrillada_audio_disc_add_pause_cb (GtkAction *action,
				 ParrilladaAudioDisc *disc)
{
	parrillada_audio_disc_add_pause (disc);
}

static void
parrillada_audio_disc_add_slices (ParrilladaAudioDisc *disc,
			       GtkTreePath *treepath,
			       GSList *slices)
{
	ParrilladaSessionCfg *session;
	ParrilladaStreamFormat format;
	ParrilladaAudioSlice *slice;
	ParrilladaTrack *track;
	GtkTreeModel *model;
	GSList *iter;
	gchar *uri;
	...
	for (iter = slices->next; iter; iter = iter->next) {
		ParrilladaTrackStream *new_track;

		slice = iter->data;

		new_track = parrillada_track_stream_new ();
		parrillada_track_stream_set_source (new_track, uri);
		parrillada_track_stream_set_format (new_track, format);
		parrillada_track_stream_set_boundaries (new_track,
						     slice->start,
						     slice->end,
						     parrillada_track_stream_get_gap (PARRILLADA_TRACK_STREAM (track)));
		parrillada_track_tag_copy_missing (PARRILLADA_TRACK (new_track),
						PARRILLADA_TRACK (track));
...
parrillada_audio_disc_split (ParrilladaAudioDisc *disc)
{
	...
	uri = parrillada_track_stream_get_source (PARRILLADA_TRACK_STREAM (track), TRUE);
	parrillada_split_dialog_set_uri (PARRILLADA_SPLIT_DIALOG (dialog),
	                              uri,
	...
	parrillada_split_dialog_set_boundaries (PARRILLADA_SPLIT_DIALOG (dialog),
	...
	slices = parrillada_split_dialog_get_slices (PARRILLADA_SPLIT_DIALOG (dialog));
	gtk_widget_destroy (dialog);

	parrillada_audio_disc_add_slices (disc, treepath, slices);
...
static void
parrillada_audio_disc_split_cb (GtkAction *action,
			     ParrilladaAudioDisc *disc)
{
	parrillada_audio_disc_split (disc);
}

static void
parrillada_audio_disc_selection_changed (GtkTreeSelection *selection,
				      ParrilladaAudioDisc *disc)
{
	...
	parrillada_disc_selection_changed (PARRILLADA_DISC (disc));
...
/********************************** Menus **************************************/
static void
parrillada_audio_disc_open_file (ParrilladaAudioDisc *disc)
{
	char *uri;
	GList *item, *list;
	GSList *uris = NULL;
...
static gboolean
parrillada_audio_disc_rename_songs (GtkTreeModel *model,
				 GtkTreeIter *iter,
				 GtkTreePath *treepath,
					       ...
					       GList *list)
{
	...
	parrillada_multi_song_props_set_rename_callback (PARRILLADA_MULTI_SONG_PROPS (props),
						      ...
						      parrillada_audio_disc_rename_songs);
...
static void
parrillada_audio_disc_edit_single_song_properties (ParrilladaAudioDisc *disc,
						GtkTreePath *treepath)
{
	gint64 gap;
	gint isrc;
	...
	if (!track)
        ...
        gtk_tree_model_get_iter (model, &iter, treepath);
        gtk_tree_model_get (model, &iter,
                            PARRILLADA_VIDEO_TREE_MODEL_INDEX_NUM, &track_num,
                            -1);
...
static void
parrillada_audio_disc_edit_song_properties (ParrilladaAudioDisc *disc,
					 GList *list)
{
	GList *item;
	gint song_num;
	...
	if (song_num == 1)
		parrillada_audio_disc_edit_single_song_properties (disc, real_list->data);
	else
		parrillada_audio_disc_edit_multi_song_properties (disc, real_list);
...
parrillada_audio_disc_open_activated_cb (GtkAction *action,
				      ParrilladaAudioDisc *disc)
{
	parrillada_audio_disc_open_file (disc);
}
					...
					ParrilladaAudioDisc *disc)
{
	...
	parrillada_audio_disc_edit_song_properties (disc, list);
...
parrillada_audio_disc_delete_activated_cb (GtkAction *action,
					ParrilladaDisc *disc)
{
	parrillada_audio_disc_delete_selected (disc);
}

static void
parrillada_audio_disc_clipboard_text_cb (GtkClipboard *clipboard,
				      const gchar *text,
				      ParrilladaAudioDisc *disc)
{
	gchar **array;
...
static void
parrillada_audio_disc_clipboard_targets_cb (GtkClipboard *clipboard,
					 GdkAtom *atoms,
					 gint n_atoms,
					 ParrilladaAudioDisc *disc)
{
	if (parrillada_clipboard_selection_may_have_uri (atoms, n_atoms))
		gtk_clipboard_request_text (clipboard,
					    (GtkClipboardTextReceivedFunc) parrillada_audio_disc_clipboard_text_cb,
					    disc);
}
				       ...
				       ParrilladaAudioDisc *disc)
{
	...
	gtk_clipboard_request_targets (clipboard,
				       (GtkClipboardTargetsReceivedFunc) parrillada_audio_disc_clipboard_targets_cb,
				       disc);
}
...
/********************************** key press event ****************************/
static void
parrillada_audio_disc_rename_activated (ParrilladaAudioDisc *disc)
{
	GtkTreeSelection *selection;
	GtkTreeViewColumn *column;
	GtkTreePath *treepath;
				    ...
				    ParrilladaAudioDisc *disc)
{
	...
	if (event->keyval == GDK_KEY_KP_Delete || event->keyval == GDK_KEY_Delete) {
		parrillada_audio_disc_delete_selected (PARRILLADA_DISC (disc));
	}
	else if (event->keyval == GDK_KEY_F2)
		parrillada_audio_disc_rename_activated (disc);
				     ...
				     gchar **uri)
{
	...
	model = gtk_tree_view_get_model (GTK_TREE_VIEW (audio->priv->tree));
        track = parrillada_video_tree_model_path_to_track (PARRILLADA_VIDEO_TREE_MODEL (model),
...
static void
parrillada_audio_disc_class_init (ParrilladaAudioDiscClass *klass)
{
	GObjectClass *object_class = G_OBJECT_CLASS(klass);

	parent_class = g_type_class_peek_parent (klass);
	object_class->finalize = parrillada_audio_disc_finalize;
	object_class->set_property = parrillada_audio_disc_set_property;
	object_class->get_property = parrillada_audio_disc_get_property;
...
GtkWidget *
parrillada_audio_disc_new ()
{
	ParrilladaAudioDisc *obj;
	
	obj = PARRILLADA_AUDIO_DISC (g_object_new (PARRILLADA_TYPE_AUDIO_DISC, NULL));
