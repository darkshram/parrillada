...
#include "parrillada-time-button.h"
...
G_DEFINE_TYPE (ParrilladaSongProps, parrillada_song_props, GTK_TYPE_DIALOG);

struct ParrilladaSongPropsPrivate {
       	GtkWidget *title;
	GtkWidget *artist;
	GtkWidget *composer;
	GtkWidget *isrc;
...
static void
parrillada_song_props_update_length (ParrilladaSongProps *self)
{
	gchar *length_str;
	gint64 start;
	gint64 end;
	...
	end = parrillada_time_button_get_value (PARRILLADA_TIME_BUTTON (self->priv->end));
	start = parrillada_time_button_get_value (PARRILLADA_TIME_BUTTON (self->priv->start));
	gap = gtk_spin_button_get_value (GTK_SPIN_BUTTON (self->priv->gap)) * GST_SECOND;

	length_str = parrillada_units_get_time_string (PARRILLADA_STREAM_LENGTH (start, end + gap), TRUE, FALSE);
	gtk_label_set_markup (GTK_LABEL (self->priv->length), length_str);
	g_free (length_str);
}

static void
parrillada_song_props_end_changed_cb (ParrilladaTimeButton *button,
				   ParrilladaSongProps *self)
{
	gint64 end;

	end = parrillada_time_button_get_value (PARRILLADA_TIME_BUTTON (self->priv->end));
	parrillada_time_button_set_max (PARRILLADA_TIME_BUTTON (self->priv->start), end - 1);
	parrillada_song_props_update_length (self);
}

static void
parrillada_song_props_start_changed_cb (ParrilladaTimeButton *button,
				     ParrilladaSongProps *self)
{
	parrillada_song_props_update_length (self);
}
...
static void
parrillada_song_props_gap_changed_cb (GtkSpinButton *button,
				   ParrilladaSongProps *self)
{
	gchar *length_str;
	gint64 start;
...
static void
parrillada_song_props_title_set (GtkEntry *entry,
			      ParrilladaSongProps *self)
{
...
static void
parrillada_song_props_artist_set (GtkEntry *entry,
			       ParrilladaSongProps *self)
{
...
static void
parrillada_song_props_composer_set (GtkEntry *entry,
				 ParrilladaSongProps *self)
{
...
static void
parrillada_song_props_init (ParrilladaSongProps *obj)
{
	gchar *title_str;
	GtkWidget *label;
	GtkWidget *table;
	...
	obj->priv = g_new0 (ParrilladaSongPropsPrivate, 1);
	...
	frame = parrillada_utils_pack_properties ("", table, NULL);
	...
	obj->priv->label = parrillada_utils_properties_get_label (frame);
	...
	g_signal_connect (obj->priv->title,
			  "changed",
			  G_CALLBACK (parrillada_song_props_title_set),
			  obj);
	...
	g_signal_connect (obj->priv->artist,
			  "changed",
			  G_CALLBACK (parrillada_song_props_artist_set),
			  obj);
	...
	g_signal_connect (obj->priv->composer,
			  "changed",
			  G_CALLBACK (parrillada_song_props_composer_set),
			  obj);
	...
	frame = parrillada_utils_pack_properties (title_str,
					       table,
					       NULL);
	g_free (title_str);
	...
	g_signal_connect (obj->priv->end,
			  "value-changed",
			  G_CALLBACK (parrillada_song_props_end_changed_cb),
			  obj);
	g_signal_connect (obj->priv->start,
			  "value-changed",
			  G_CALLBACK (parrillada_song_props_start_changed_cb),
			  obj);
	g_signal_connect (obj->priv->gap,
			  "value-changed",
			  G_CALLBACK (parrillada_song_props_gap_changed_cb),
			  obj);
...
static gchar *
parrillada_song_props_get_entry_value (GtkEntry *entry)
{
	const gchar *text;
				   ...
				   gint64 *gap)
{
	if (artist) {
		if (self->priv->artist_set)
			*artist = parrillada_song_props_get_entry_value (GTK_ENTRY (self->priv->artist));
		else
			*artist = NULL;
	}
	...
	if (title) {
		if (self->priv->title_set)
			*title = parrillada_song_props_get_entry_value (GTK_ENTRY (self->priv->title));
		else
			*title = NULL;
	}
	...
	if (composer) {
		if (self->priv->composer_set)
			*composer = parrillada_song_props_get_entry_value (GTK_ENTRY (self->priv->composer));
		else
			*composer = NULL;
	}
	...
	if (isrc) {
		...
		string = parrillada_song_props_get_entry_value (GTK_ENTRY (self->priv->isrc));
		if (string)
				   ...
				   gint64 gap)
{
	...
	if (track_num >= 0) {
		...
		parrillada_time_button_set_show_frames (PARRILLADA_TIME_BUTTON (self->priv->start), FALSE);
		parrillada_time_button_set_show_frames (PARRILLADA_TIME_BUTTON (self->priv->end), FALSE);
	...
	parrillada_time_button_set_max (PARRILLADA_TIME_BUTTON (self->priv->start), end - 1);
	parrillada_time_button_set_value (PARRILLADA_TIME_BUTTON (self->priv->start), start);

	g_signal_handlers_block_by_func (self->priv->end,
					 parrillada_song_props_end_changed_cb,
					 self);
	parrillada_time_button_set_max (PARRILLADA_TIME_BUTTON (self->priv->end), start + length);
	parrillada_time_button_set_value (PARRILLADA_TIME_BUTTON (self->priv->end), end);
...
static void
parrillada_song_props_finalize (GObject *object)
{
	ParrilladaSongProps *cobj;

	cobj = PARRILLADA_SONG_PROPS(object);
...
static void
parrillada_song_props_class_init (ParrilladaSongPropsClass *klass)
{
	GObjectClass *object_class = G_OBJECT_CLASS(klass);

	parent_class = g_type_class_peek_parent(klass);
	object_class->finalize = parrillada_song_props_finalize;
}

GtkWidget *
parrillada_song_props_new ()
{
	ParrilladaSongProps *obj;
	
	obj = PARRILLADA_SONG_PROPS (g_object_new (PARRILLADA_TYPE_SONG_PROPS, NULL));
