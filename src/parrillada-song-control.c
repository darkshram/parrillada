...
typedef struct _ParrilladaSongControlPrivate ParrilladaSongControlPrivate;
struct _ParrilladaSongControlPrivate
{
	GstElement *pipe;
	GstState state;
...
#define PARRILLADA_SONG_CONTROL_PRIVATE(o)  (G_TYPE_INSTANCE_GET_PRIVATE ((o), PARRILLADA_TYPE_SONG_CONTROL, ParrilladaSongControlPrivate))

G_DEFINE_TYPE (ParrilladaSongControl, parrillada_song_control, GTK_TYPE_ALIGNMENT);

gint64
parrillada_song_control_get_length (ParrilladaSongControl *self)
{
	ParrilladaSongControlPrivate *priv;

	priv = PARRILLADA_SONG_CONTROL_PRIVATE (self);
	if (!priv->uri)
		return -1;
...
gint64
parrillada_song_control_get_pos (ParrilladaSongControl *self)
{
	ParrilladaSongControlPrivate *priv;
...
parrillada_song_control_update_position (ParrilladaSongControl *player)
{
	...
	pos_string = parrillada_units_get_time_string (value, FALSE, FALSE);
...
static gboolean
parrillada_song_control_set_pos (ParrilladaSongControl *player,
                              gint64 pos)
{
	ParrilladaSongControlPrivate *priv;
...
static void
parrillada_song_control_range_value_changed (GtkRange *range,
                                          ParrilladaSongControl *player)
{
	ParrilladaSongControlPrivate *priv;
	...
	if (priv->state >= GST_STATE_PAUSED && !priv->update_scale_id) {
		...
		parrillada_song_control_set_pos (PARRILLADA_SONG_CONTROL (player), (gint64) pos + priv->start);
	}

	parrillada_song_control_update_position (player);
}

static void
parrillada_song_control_set_length (ParrilladaSongControl *player)
{
	ParrilladaSongControlPrivate *priv;
...
static gboolean
parrillada_song_control_range_button_pressed_cb (GtkWidget *widget,
                                              GdkEvent *event,
                                              ParrilladaSongControl *player)
{
	ParrilladaSongControlPrivate *priv;
...
static gboolean
parrillada_song_control_range_button_released_cb (GtkWidget *widget,
                                               GdkEvent *event,
                                               ParrilladaSongControl *player)
{
	ParrilladaSongControlPrivate *priv;
	...
	if (priv->state == GST_STATE_PLAYING && !priv->update_scale_id)
		priv->update_scale_id = g_timeout_add (500,
		                                       (GSourceFunc) parrillada_song_control_update_progress_cb,
		                                       player);
...
static void
parrillada_song_control_button_clicked_cb (GtkButton *button,
                                        ParrilladaSongControl *player)
{
	ParrilladaSongControlPrivate *priv;
...
static void
parrillada_song_control_volume_changed_cb (GtkScaleButton *button,
                                        gdouble volume,
                                        ParrilladaSongControl *player)
{
	ParrilladaSongControlPrivate *priv;
...
const gchar *
parrillada_song_control_get_uri (ParrilladaSongControl *player)
{
	ParrilladaSongControlPrivate *priv;
                               ...
                               const gchar *artist)
{
	...
	parrillada_song_control_set_length (player);
	...
	if (artist) {
		...
		gchar *name;
	    	...
	    	PARRILLADA_GET_BASENAME_FOR_DISPLAY (priv->uri, name);
                                     ...
                                     gsize end)
{
	...
	parrillada_song_control_set_pos (player, start);
}
...
static gboolean
parrillada_song_control_bus_messages (GstBus *bus,
				   GstMessage *msg,
				   ParrilladaSongControl *player)
{
	ParrilladaSongControlPrivate *priv;
	...
	case GST_MESSAGE_EOS:
		...
		gst_element_set_state (priv->pipe, GST_STATE_PAUSED);
		parrillada_song_control_set_pos (player, priv->start);
		break;
	...
	case GST_MESSAGE_STATE_CHANGED:
		...
		if (state == GST_STATE_PLAYING) {
			if (priv->state == GST_STATE_READY) {
				...
				pos = gtk_range_get_value (GTK_RANGE (priv->progress));
				parrillada_song_control_set_pos (player, priv->start + pos);
...
parrillada_song_control_destroy (GtkObject *obj)
{
	...
	priv = PARRILLADA_SONG_CONTROL_PRIVATE (obj);
...
static void
parrillada_song_control_init (ParrilladaSongControl *object)
{
	ParrilladaSongControlPrivate *priv;
	GtkWidget *alignment;
	GtkWidget *volume;
	...
	priv = PARRILLADA_SONG_CONTROL_PRIVATE (object);
	...
	gst_bus_add_watch (bus,
			   (GstBusFunc) parrillada_song_control_bus_messages,
			   object);
	...
	g_signal_connect (G_OBJECT (priv->progress),
			  "button-press-event",
			  G_CALLBACK (parrillada_song_control_range_button_pressed_cb), object);
	g_signal_connect (G_OBJECT (priv->progress),
			  "button-release-event",
			  G_CALLBACK (parrillada_song_control_range_button_released_cb), object);
	g_signal_connect (G_OBJECT (priv->progress),
			  "value-changed",
			  G_CALLBACK (parrillada_song_control_range_value_changed),
			  object);
	...
	g_signal_connect (volume,
			  "value-changed",
			  G_CALLBACK (parrillada_song_control_volume_changed_cb),
			  object);
...
static void
parrillada_song_control_finalize (GObject *object)
{
	G_OBJECT_CLASS (parrillada_song_control_parent_class)->finalize (object);
}
...
{
	return g_object_new (PARRILLADA_TYPE_SONG_CONTROL, NULL);
}
