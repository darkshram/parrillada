...
static void parrillada_search_entry_class_init (ParrilladaSearchEntryClass *klass);
static void parrillada_search_entry_init (ParrilladaSearchEntry *sp);
static void parrillada_search_entry_finalize (GObject *object);
static void parrillada_search_entry_destroy (GtkObject *gtk_object);

struct ParrilladaSearchEntryPrivate {
	GtkWidget *button;
	GtkWidget *combo;
	gint search_id;

	ParrilladaLayoutType ctx;
...
enum {
	PARRILLADA_SEARCH_ENTRY_DISPLAY_COL,
	PARRILLADA_SEARCH_ENTRY_BACKGRD_COL,
	PARRILLADA_SEARCH_ENTRY_NB_COL
};

static GObjectClass *parent_class = NULL;

#define PARRILLADA_SEARCH_ENTRY_MAX_HISTORY_ITEMS	10

static void parrillada_search_entry_button_clicked_cb (GtkButton *button,
						      ParrilladaSearchEntry *entry);
static void parrillada_search_entry_entry_activated_cb (GtkComboBox *combo,
						      ParrilladaSearchEntry *entry);
static void parrillada_search_entry_activated (ParrilladaSearchEntry *entry, gboolean query_now);
static void parrillada_search_entry_set_history (ParrilladaSearchEntry *entry,
                                              const gchar * const *history);
static void parrillada_search_entry_save_history (ParrilladaSearchEntry *entry);
static void parrillada_search_entry_add_current_keyword_to_history (ParrilladaSearchEntry *entry);
static void parrillada_search_entry_category_clicked_cb (GtkWidget *button, ParrilladaSearchEntry *entry);

typedef enum {
	ACTIVATED_SIGNAL,
	LAST_SIGNAL
} ParrilladaSearchEntrySignalType;

static guint parrillada_search_entry_signals[LAST_SIGNAL] = { 0 };

GType
parrillada_search_entry_get_type ()
{
	static GType type = 0;

	if (type == 0) {
		static const GTypeInfo our_info = {
			sizeof (ParrilladaSearchEntryClass),
			NULL,
			NULL,
			(GClassInitFunc) parrillada_search_entry_class_init,
			NULL,
			NULL,
			sizeof (ParrilladaSearchEntry),
			0,
			(GInstanceInitFunc) parrillada_search_entry_init,
		};
...
static void
parrillada_search_entry_class_init (ParrilladaSearchEntryClass *klass)
{
	GObjectClass *object_class = G_OBJECT_CLASS (klass);
	GtkObjectClass *gtk_object_class = GTK_OBJECT_CLASS (klass);

	parent_class = g_type_class_peek_parent (klass);
	object_class->finalize = parrillada_search_entry_finalize;
	gtk_object_class->destroy = parrillada_search_entry_destroy;

	parrillada_search_entry_signals[ACTIVATED_SIGNAL] =
	    g_signal_new ("activated", G_OBJECT_CLASS_TYPE (object_class),
			  G_SIGNAL_RUN_FIRST | G_SIGNAL_ACTION,
			  G_STRUCT_OFFSET (ParrilladaSearchEntryClass,
					   activated), NULL, NULL,
			  g_cclosure_marshal_VOID__VOID, G_TYPE_NONE, 0);
}

static gboolean
parrillada_search_entry_separator_func (GtkTreeModel *model,
				     GtkTreeIter *iter,
				     gpointer data)
{
	gchar *display;
	...
	gtk_tree_model_get (model, iter,
			    PARRILLADA_SEARCH_ENTRY_DISPLAY_COL, &display,
			    -1);
...
static void
parrillada_search_entry_init (ParrilladaSearchEntry *obj)
{
	gchar *string;
	gpointer value;
	GtkWidget *table;
	...
	obj->priv = g_new0 (ParrilladaSearchEntryPrivate, 1);
	...
	g_signal_connect (G_OBJECT (obj->priv->button),
			  "clicked",
			  G_CALLBACK (parrillada_search_entry_button_clicked_cb),
			  obj);

	/* Set up the combo box */
	store = gtk_list_store_new (PARRILLADA_SEARCH_ENTRY_NB_COL,
				    G_TYPE_STRING,
				    G_TYPE_STRING);
	...
	g_signal_connect (G_OBJECT (obj->priv->combo),
			  "changed",
			  G_CALLBACK (parrillada_search_entry_entry_activated_cb),
			  obj);

	gtk_combo_box_set_row_separator_func (GTK_COMBO_BOX (obj->priv->combo),
					      parrillada_search_entry_separator_func,
					      obj,
					      NULL);
	...
	gtk_cell_layout_set_attributes (GTK_CELL_LAYOUT (obj->priv->combo),
					renderer,
					"text", PARRILLADA_SEARCH_ENTRY_DISPLAY_COL,
					"background", PARRILLADA_SEARCH_ENTRY_BACKGRD_COL,
					NULL);
	...
	gtk_entry_completion_set_text_column (completion, PARRILLADA_SEARCH_ENTRY_DISPLAY_COL);
	...
	g_signal_connect (obj->priv->documents,
			  "clicked",
			  G_CALLBACK (parrillada_search_entry_category_clicked_cb),
			  obj);
	...
	parrillada_search_entry_set_history (obj, value);
	g_strfreev (value);
}

static void
parrillada_search_entry_destroy (GtkObject *gtk_object)
{
	GTK_OBJECT_CLASS (parent_class)->destroy (gtk_object);
}

static void
parrillada_search_entry_finalize (GObject *object)
{
	ParrilladaSearchEntry *cobj;

	cobj = PARRILLADA_SEARCH_ENTRY (object);
...
GtkWidget *
parrillada_search_entry_new ()
{
	ParrilladaSearchEntry *obj;

	obj = PARRILLADA_SEARCH_ENTRY (g_object_new (PARRILLADA_TYPE_SEARCH_ENTRY, NULL));

	return GTK_WIDGET (obj);
}

static void
parrillada_search_entry_category_clicked_cb (GtkWidget *button,
					  ParrilladaSearchEntry *entry)
{
...
static void
parrillada_search_entry_button_clicked_cb (GtkButton *button,
					ParrilladaSearchEntry *entry)
{
	parrillada_search_entry_activated (entry, TRUE);
}

static void
parrillada_search_entry_entry_activated_cb (GtkComboBox *combo,
					 ParrilladaSearchEntry *entry)
{
	if (gtk_combo_box_get_active (GTK_COMBO_BOX (entry->priv->combo)) != -1)
		parrillada_search_entry_activated (entry, TRUE);
	else
		parrillada_search_entry_activated (entry, FALSE);
}

static void
parrillada_search_entry_check_keywords (ParrilladaSearchEntry *entry) 
{
	const char *keywords;
	char *tmp;
	...
	g_signal_emit (entry,
		       parrillada_search_entry_signals [ACTIVATED_SIGNAL],
		       0);
	parrillada_search_entry_add_current_keyword_to_history (entry);
...
static gboolean
parrillada_search_entry_activated_real (ParrilladaSearchEntry *entry)
{
	entry->priv->search_id = 0;
	parrillada_search_entry_check_keywords (entry);
	return FALSE;
}

static void
parrillada_search_entry_activated (ParrilladaSearchEntry *entry,
				gboolean search_now)
{
	/* we reset the timer */
	...
	if (!search_now) {
		...
		entry->priv->search_id = g_timeout_add_seconds (2,
								(GSourceFunc) parrillada_search_entry_activated_real,
								entry);
	}
	else
		parrillada_search_entry_check_keywords (entry);
...
static void
parrillada_search_entry_set_history (ParrilladaSearchEntry *entry,
                                  const gchar * const *history)
{
	int i;
	GtkTreeIter row;
	...
	if (history) {
		for (i = 0; history [i] && i < PARRILLADA_SEARCH_ENTRY_MAX_HISTORY_ITEMS; i ++) {
			gtk_list_store_append (store, &row);
			gtk_list_store_set (store, &row,
					    PARRILLADA_SEARCH_ENTRY_DISPLAY_COL, history [i],
					    PARRILLADA_SEARCH_ENTRY_BACKGRD_COL, NULL,
					    -1);
		}
	}
	...
	gtk_list_store_append (store, &row);
	gtk_list_store_set (store, &row,
			    PARRILLADA_SEARCH_ENTRY_DISPLAY_COL, NULL,
			    PARRILLADA_SEARCH_ENTRY_BACKGRD_COL, NULL,
			    -1);
	...
	gtk_list_store_append (store, &row);
	gtk_list_store_set (store, &row,
			    PARRILLADA_SEARCH_ENTRY_DISPLAY_COL, _("All files"),
			    PARRILLADA_SEARCH_ENTRY_BACKGRD_COL, NULL,
			    //PARRILLADA_SEARCH_ENTRY_BACKGRD_COL, "grey90",
			    -1);
}

static void
parrillada_search_entry_save_history (ParrilladaSearchEntry *entry)
{
	GtkTreeModel *model;
	int num_children;
	GtkTreeIter iter;
	...
	do {
		...
		string = NULL;
		gtk_tree_model_get (model, &iter,
		                    PARRILLADA_SEARCH_ENTRY_DISPLAY_COL, &string,
		                    -1);
...
static void
parrillada_search_entry_add_current_keyword_to_history (ParrilladaSearchEntry *entry)
{
	...
	if (gtk_tree_model_iter_n_children (model, NULL) == PARRILLADA_SEARCH_ENTRY_MAX_HISTORY_ITEMS + 2) {
		gtk_tree_model_iter_nth_child (model,
		                               &iter,
		                               NULL, 
		                               PARRILLADA_SEARCH_ENTRY_MAX_HISTORY_ITEMS - 1);
	...
	gtk_list_store_prepend (GTK_LIST_STORE (model), &iter);
	gtk_list_store_set (GTK_LIST_STORE (model), &iter,
	                    PARRILLADA_SEARCH_ENTRY_DISPLAY_COL, keywords,
	                    -1);

end:

	parrillada_search_entry_save_history (entry);
}
...
{
	ParrilladaSearchScope scope = PARRILLADA_SEARCH_SCOPE_ANY;
	...
	if (gtk_toggle_button_get_active (GTK_TOGGLE_BUTTON (entry->priv->documents)))
		scope |= PARRILLADA_SEARCH_SCOPE_DOCUMENTS;

	if (gtk_toggle_button_get_active (GTK_TOGGLE_BUTTON (entry->priv->pictures)))
		scope |= PARRILLADA_SEARCH_SCOPE_PICTURES;

	if (gtk_toggle_button_get_active (GTK_TOGGLE_BUTTON (entry->priv->music)))
		scope |= PARRILLADA_SEARCH_SCOPE_MUSIC;

	if (gtk_toggle_button_get_active (GTK_TOGGLE_BUTTON (entry->priv->video)))
		scope |= PARRILLADA_SEARCH_SCOPE_VIDEO;

	parrillada_search_engine_new_query (search, keywords);
	parrillada_search_engine_set_query_scope (search, scope);

	return TRUE;
}

void
parrillada_search_entry_set_context (ParrilladaSearchEntry *self,
