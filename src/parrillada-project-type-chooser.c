...
typedef enum {
	...
	CHOSEN_SIGNAL,
	LAST_SIGNAL
} ParrilladaProjectTypeChooserSignalType;
static guint parrillada_project_type_chooser_signals [LAST_SIGNAL] = { 0 };

enum {
	PARRILLADA_PROJECT_TYPE_CHOOSER_ICON_COL,
	PARRILLADA_PROJECT_TYPE_CHOOSER_TEXT_COL,
	PARRILLADA_PROJECT_TYPE_CHOOSER_ID_COL,
	PARRILLADA_PROJECT_TYPE_CHOOSER_NB_COL
};
...
static void
parrillada_project_type_chooser_button_clicked (GtkButton *button,
...
static GtkWidget *
parrillada_project_type_chooser_new_item (ParrilladaProjectTypeChooser *chooser,
				       ItemDescription *description)
{
	...
	g_signal_connect (eventbox,
			  "clicked",
			  G_CALLBACK (parrillada_project_type_chooser_button_clicked),
			  chooser);
...
static void
parrillada_project_type_chooser_recent_clicked_cb (GtkButton *button,
						ParrilladaProjectTypeChooser *self)
{
	const gchar *uri;

	uri = g_object_get_data (G_OBJECT (button), "ParrilladaButtonURI");
	g_signal_emit (self,
		       parrillada_project_type_chooser_signals [RECENT_CLICKED_SIGNAL],
		       0,
		       uri);
}

static void
parrillada_project_type_chooser_last_unsaved_clicked_cb (GtkButton *button,
						      ParrilladaProjectTypeChooser *self)
{
	const gchar *uri;
	gchar *path;
	...
	g_signal_emit (self,
		       parrillada_project_type_chooser_signals [LAST_SAVED_CLICKED_SIGNAL],
		       0,
		       path);
...
static gint
parrillada_project_type_chooser_sort_recent (gconstpointer a, gconstpointer b)
{
...
static void
parrillada_project_type_chooser_build_recent (ParrilladaProjectTypeChooser *self,
					   GtkRecentManager *recent)
{
	...
	for (iter = recents; iter; iter = iter->next) {
		...
		list = g_list_insert_sorted (list,
					     info,
					     parrillada_project_type_chooser_sort_recent);
	...
	filename = g_build_filename (g_get_user_config_dir (),
				     "parrillada",
				     PARRILLADA_SESSION_TMP_PROJECT_PATH,
				     NULL);
	if (g_file_test (filename, G_FILE_TEST_EXISTS)) {
		...
		image = gtk_image_new_from_icon_name ("parrillada", GTK_ICON_SIZE_BUTTON);
		...
		g_signal_connect (link,
				  "clicked",
				  G_CALLBACK (parrillada_project_type_chooser_last_unsaved_clicked_cb),
				  self);
	...
	for (iter = list; iter; iter = iter->next) {
		...
		g_object_set_data_full (G_OBJECT (link),
					"ParrilladaButtonURI", g_strdup (uri),
					g_free);
		...
		g_signal_connect (link,
				  "clicked",
				  G_CALLBACK (parrillada_project_type_chooser_recent_clicked_cb),
				  self);
		gtk_widget_show (link);
...
static void
parrillada_project_type_chooser_recent_changed_cb (GtkRecentManager *recent,
						ParrilladaProjectTypeChooser *self)
{
	gtk_container_foreach (GTK_CONTAINER (self->priv->recent_box),
			       (GtkCallback) gtk_widget_destroy,
			       NULL);
	parrillada_project_type_chooser_build_recent (self, recent);
}

static void
parrillada_project_type_chooser_init (ParrilladaProjectTypeChooser *obj)
{
	GtkRecentManager *recent;
	...
	for (i = 0; i < nb_items; i ++) {
		widget = parrillada_project_type_chooser_new_item (obj, items + i);
	...
	parrillada_project_type_chooser_build_recent (obj, recent);

	g_signal_connect (recent,
			  "changed",
			  G_CALLBACK (parrillada_project_type_chooser_recent_changed_cb),
			  obj);
}
...
parrillada_project_type_chooser_class_init (ParrilladaProjectTypeChooserClass *klass)
{
	...
	parrillada_project_type_chooser_signals[CHOSEN_SIGNAL] =
	    g_signal_new ("chosen", G_OBJECT_CLASS_TYPE (object_class),
			  ...
			  G_STRUCT_OFFSET (ParrilladaProjectTypeChooserClass, chosen),
			  NULL, NULL,
	...
	parrillada_project_type_chooser_signals[RECENT_CLICKED_SIGNAL] =
	    g_signal_new ("recent_clicked", G_OBJECT_CLASS_TYPE (object_class),
			  G_SIGNAL_ACTION | G_SIGNAL_RUN_FIRST,
			  G_STRUCT_OFFSET (ParrilladaProjectTypeChooserClass, recent_clicked),
			  NULL, NULL,
	...
	parrillada_project_type_chooser_signals[LAST_SAVED_CLICKED_SIGNAL] =
	    g_signal_new ("last_saved", G_OBJECT_CLASS_TYPE (object_class),
			  G_SIGNAL_ACTION | G_SIGNAL_RUN_FIRST,
			  G_STRUCT_OFFSET (ParrilladaProjectTypeChooserClass, last_saved_clicked),
			  NULL, NULL,
			  g_cclosure_marshal_VOID__STRING,
			  G_TYPE_NONE,
			  1,
...
GtkWidget *
parrillada_project_type_chooser_new ()
{
	ParrilladaProjectTypeChooser *obj;

	obj = PARRILLADA_PROJECT_TYPE_CHOOSER (g_object_new (PARRILLADA_TYPE_PROJECT_TYPE_CHOOSER,
					    NULL));
