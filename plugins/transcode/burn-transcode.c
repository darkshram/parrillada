                                  ...
                                  gpointer user_data)
{
	...
	ParrilladaTranscode *self = user_data;
				   ...
				   GError **error)
{
	...
	&& (parrillada_track_stream_get_format (PARRILLADA_TRACK_STREAM (track)) & PARRILLADA_AUDIO_FORMAT_DTS) != 0) {
		...
		priv->probe = gst_pad_add_probe (sinkpad, GST_PAD_PROBE_TYPE_BUFFER,
		                                 parrillada_transcode_buffer_handler,
		                                 transcode, NULL);
	...
	if (action == PARRILLADA_JOB_ACTION_IMAGE) {
		...
		filtercaps = gst_caps_new_full (gst_structure_new ("audio/x-raw",
								   ...
								   "format", G_TYPE_STRING, (session_format & PARRILLADA_AUDIO_FORMAT_RAW_LITTLE_ENDIAN) != 0 ? "S16LE" : "S16BE",
...
parrillada_transcode_get_duration (ParrilladaTranscode *transcode)
{
	...
	PARRILLADA_JOB_LOG (transcode, "got duration %" GST_TIME_FORMAT, GST_TIME_ARGS (duration));
	     ...
	     ParrilladaTranscode *transcode)
{
	...
	if (!strcmp (tag, GST_TAG_TITLE)) {
		...
		if (!parrillada_track_tag_lookup_string (track, PARRILLADA_TRACK_STREAM_ISRC_TAG)) {
			gchar *isrc = NULL;
