...
typedef struct _ParrilladaMkisofsBase ParrilladaMkisofsBase;

struct _ParrilladaWriteGraftData {
	ParrilladaMkisofsBase *base;
	GError **error;
};
typedef struct _ParrilladaWriteGraftData ParrilladaWriteGraftData;

static void
parrillada_mkisofs_base_clean (ParrilladaMkisofsBase *base)
{
...
static ParrilladaBurnResult
parrillada_mkisofs_base_write_excluded (ParrilladaMkisofsBase *base,
				     const gchar *uri,
				     GError **error)
{
	gint num;
	...
	if (uri && uri [0] == '/') {
		...
		PARRILLADA_BURN_LOG ("File not stored locally %s", uri);
		g_set_error (error,
			     PARRILLADA_BURN_ERROR,
	...
	if (!localpath) {
		PARRILLADA_BURN_LOG ("Localpath is NULL");
		return PARRILLADA_BURN_ERR;
	}
	...
	if (forbidden) {
		...
		PARRILLADA_BURN_LOG ("Escaped path %s into %s", localpath, tmp);
		g_free (localpath);
		localpath = tmp;
	}
...
static ParrilladaBurnResult
parrillada_mkisofs_base_write_graft (ParrilladaMkisofsBase *base,
				  const gchar *uri,
		       ...
		       ParrilladaWriteGraftData *data)
{
	...
	for (; grafts; grafts = grafts->next) {
		...
		if (!graft->path) {
			result = parrillada_mkisofs_base_write_graft (data->base,
								   graft->uri,
								   NULL,
								   data->error);
			if (result != PARRILLADA_BURN_OK)
...
static ParrilladaBurnResult
parrillada_mkisofs_base_write_grafts (ParrilladaMkisofsBase *base,
				   GError **error)
{
	ParrilladaWriteGraftData callback_data;
	gpointer result;
...
static ParrilladaBurnResult
parrillada_mkisofs_base_create_video_empty (ParrilladaMkisofsBase *base,
					 const gchar *disc_path)
{
	gchar *path;

	PARRILLADA_BURN_LOG ("Creating an empty Video or Audio directory");
...
static ParrilladaBurnResult
parrillada_mkisofs_base_empty_directory (ParrilladaMkisofsBase *base,
				      const gchar *disc_path,
				      GError **error)
{
	ParrilladaBurnResult result;
	...
	if (base->videodir) {
		...
		||  !strcmp (disc_path, "VIDEO_TS/")
		||  !strcmp (disc_path, "/VIDEO_TS/")) {
			parrillada_mkisofs_base_create_video_empty (base, disc_path);
...
static ParrilladaBurnResult
parrillada_mkisofs_base_process_video_graft (ParrilladaMkisofsBase *base,
					  ParrilladaGraftPt *graft,
					  GError **error)
{
	gchar *link_path;
	...
	PARRILLADA_BURN_LOG ("Linking %s to %s", link_path, path);
...
static ParrilladaBurnResult
parrillada_mkisofs_base_add_graft (ParrilladaMkisofsBase *base,
				ParrilladaGraftPt *graft,
				GError **error)
{
	GSList *list;
	...
	&& !g_str_has_prefix (graft->uri, "file://")) {
		...
		PARRILLADA_BURN_LOG ("File not stored locally %s", graft->uri);
		g_set_error (error,
			     PARRILLADA_BURN_ERROR,
			     PARRILLADA_BURN_ERROR_FILE_NOT_LOCAL,
			     _("The file is not stored locally"));
	...
	if (base->videodir) {
		...
		||  !strcmp (graft->path, "VIDEO_TS/")
		||  !strcmp (graft->path, "/VIDEO_TS/")) {
			res = parrillada_mkisofs_base_process_video_graft (base, graft, error);
		...
		||  !strcmp (graft->path, "AUDIO_TS/")
		||  !strcmp (graft->path, "/AUDIO_TS/"))
			return parrillada_mkisofs_base_process_video_graft (base, graft, error);
		...
		||  !strcmp (parent, "/VIDEO_TS/")) {
			if (!base->found_video_ts)
				parrillada_mkisofs_base_create_video_empty (base, parent);
...
parrillada_mkisofs_base_write_to_files (GSList *grafts,
				     GSList *excluded,
				     gboolean use_joliet,
				     const gchar *emptydir,
				     const gchar *videodir,
				     ...
				     GError **error)
{
	...
	if (!grafts) {
		PARRILLADA_BURN_LOG ("No graft passed");
		return PARRILLADA_BURN_ERR;
	}
	...
	for (; grafts; grafts = grafts->next) {
		...
		PARRILLADA_BURN_LOG ("New graft %s %s", graft->uri, graft->path);

		if (!graft->uri) {
			result = parrillada_mkisofs_base_empty_directory (&base,
								       graft->path,
								       error);
			if (result != PARRILLADA_BURN_OK)
				goto cleanup;
		...
		result = parrillada_mkisofs_base_add_graft (&base,
							 graft,
							 error);
		if (result != PARRILLADA_BURN_OK)
			goto cleanup;
	...
	result = parrillada_mkisofs_base_write_grafts (&base, error);
	if (result != PARRILLADA_BURN_OK)
		goto cleanup;
	...
	for (; excluded; excluded = excluded->next) {
		uri = excluded->data;
		if (!uri) {
			PARRILLADA_BURN_LOG ("NULL URI");
			continue;
		}

		result = parrillada_mkisofs_base_write_excluded (&base,
							      uri,
							      error);
		if (result != PARRILLADA_BURN_OK)
			goto cleanup;
	...
	parrillada_mkisofs_base_clean (&base);
	return PARRILLADA_BURN_OK;
