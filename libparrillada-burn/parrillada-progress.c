...
static void parrillada_burn_progress_class_init (ParrilladaBurnProgressClass *klass);
static void parrillada_burn_progress_init (ParrilladaBurnProgress *sp);
static void parrillada_burn_progress_finalize (GObject *object);

static void
parrillada_burn_progress_set_property (GObject *object,
				    guint property_id,
				    const GValue *value,
				    GParamSpec *pspec);
static void
parrillada_burn_progress_get_property (GObject *object,
				    guint property_id,
				    GValue *value,
				    GParamSpec *pspec);

struct ParrilladaBurnProgressPrivate {
	GtkWidget *progress;
	GtkWidget *action;
	GtkWidget *speed;
...
GType
parrillada_burn_progress_get_type ()
{
	static GType type = 0;

	if(type == 0) {
		static const GTypeInfo our_info = {
			sizeof (ParrilladaBurnProgressClass),
			NULL,
			NULL,
			(GClassInitFunc)parrillada_burn_progress_class_init,
			NULL,
			NULL,
			sizeof (ParrilladaBurnProgress),
			0,
...
static void
parrillada_burn_progress_class_init (ParrilladaBurnProgressClass *klass)
{
	GObjectClass *object_class = G_OBJECT_CLASS(klass);

	parent_class = g_type_class_peek_parent(klass);
	object_class->finalize = parrillada_burn_progress_finalize;
	object_class->set_property = parrillada_burn_progress_set_property;
	object_class->get_property = parrillada_burn_progress_get_property;
...
static void
parrillada_burn_progress_create_info (ParrilladaBurnProgress *obj)
{
	GtkWidget *label;
	GtkWidget *table;
...
void
parrillada_burn_progress_display_session_info (ParrilladaBurnProgress *obj,
					    glong time,
					    gint64 rate,
					    ...
					    gint written)
{
	...
	if (rate > 0) {
		...
		if (media & PARRILLADA_MEDIUM_DVD)
			speed = (gfloat) PARRILLADA_RATE_TO_SPEED_DVD (rate);
		else if (media & PARRILLADA_MEDIUM_BD)
			speed = (gfloat) PARRILLADA_RATE_TO_SPEED_BD (rate);
		else
			speed = (gfloat) PARRILLADA_RATE_TO_SPEED_CD (rate);
...
parrillada_burn_progress_set_property (GObject *object,
				    ...
				    GParamSpec *pspec)
{
	ParrilladaBurnProgress *progress;

	progress = PARRILLADA_BURN_PROGRESS (object);
	switch (property_id) {
	case PROP_SHOW_INFO:
		if (!g_value_get_boolean (value)) {
			...
			parrillada_burn_progress_create_info (progress);
		break;
...
static void
parrillada_burn_progress_init (ParrilladaBurnProgress *obj)
{
	GtkWidget *box;
	...
	parrillada_burn_progress_create_info (obj);
	
	gtk_widget_show_all (GTK_WIDGET (obj));
}

static void
parrillada_burn_progress_stop_blinking (ParrilladaBurnProgress *self)
{
...
static void
parrillada_burn_progress_finalize (GObject *object)
{
	ParrilladaBurnProgress *cobj;

	cobj = PARRILLADA_BURN_PROGRESS (object);
...
GtkWidget *
parrillada_burn_progress_new ()
{
	ParrilladaBurnProgress *obj;
	
	obj = PARRILLADA_BURN_PROGRESS (g_object_new (PARRILLADA_TYPE_BURN_PROGRESS, NULL));
	
	return GTK_WIDGET (obj);
}

static gboolean
parrillada_burn_progress_pulse_cb (ParrilladaBurnProgress *self)
{
...
static void
parrillada_burn_progress_start_blinking (ParrilladaBurnProgress *self)
{
	...
	if (!self->priv->pulse_id)
		self->priv->pulse_id = g_timeout_add (150,
						      (GSourceFunc) parrillada_burn_progress_pulse_cb,
						      self);
}

void
parrillada_burn_progress_set_status (ParrilladaBurnProgress *self,
				  ParrilladaMedia media,
				  ...
				  gint64 rate)
{
	...
	if (action_progress < 0.0) {
		...
		parrillada_burn_progress_start_blinking (self);
		return;
	}

	if (self->priv->current == PARRILLADA_BURN_ACTION_NONE) {
		gtk_progress_bar_set_text (GTK_PROGRESS_BAR (self->priv->progress), " ");
	...
	if (self->priv->pulse_id)
		parrillada_burn_progress_stop_blinking (self);
	...
	if (self->priv->current == PARRILLADA_BURN_ACTION_BLANKING) {
		if (self->priv->bytes_written)
			gtk_label_set_text (GTK_LABEL (self->priv->bytes_written), " ");
		if (self->priv->speed)
			gtk_label_set_text (GTK_LABEL (self->priv->speed), " ");
...
void
parrillada_burn_progress_set_action (ParrilladaBurnProgress *self,
				  ParrilladaBurnAction action,
				  const gchar *string)
{
	gchar *final_text;

	if (action != PARRILLADA_BURN_ACTION_NONE) {
		if (!string)
			string = parrillada_burn_action_to_string (action);

		if (self->priv->speed_label) {
			if (action == PARRILLADA_BURN_ACTION_RECORDING
	...
	if (action == PARRILLADA_BURN_ACTION_BLANKING)
		parrillada_burn_progress_start_blinking (self);
	else if (action == PARRILLADA_BURN_ACTION_FINISHED)
		parrillada_burn_progress_stop_blinking (self);
}
...
{
	parrillada_burn_progress_stop_blinking (progress);

	progress->priv->current = PARRILLADA_BURN_ACTION_NONE;
