...
typedef struct _ParrilladaFileMonitorPrivate ParrilladaFileMonitorPrivate;
struct _ParrilladaFileMonitorPrivate
{
	int notify_id;
	GIOChannel *notify;
...
#define PARRILLADA_FILE_MONITOR_PRIVATE(o)  (G_TYPE_INSTANCE_GET_PRIVATE ((o), PARRILLADA_TYPE_FILE_MONITOR, ParrilladaFileMonitorPrivate))

G_DEFINE_TYPE (ParrilladaFileMonitor, parrillada_file_monitor, G_TYPE_OBJECT);

struct _ParrilladaInotifyMovedData {
	gchar *name;
...
typedef struct _ParrilladaInotifyMovedData ParrilladaInotifyMovedData;

struct _ParrilladaInotifyFileData {
	gpointer callback_data;
	gchar *name;
};
typedef struct _ParrilladaInotifyFileData ParrilladaInotifyFileData;

struct _ParrilladaFileMonitorCancelForeach {
	gpointer callback_data;
...
typedef struct _ParrilladaFileMonitorCancelForeach ParrilladaFileMonitorCancelForeach;

struct _ParrilladaFileMonitorSearchResult {
	gpointer key;
	gpointer callback_data;
};
typedef struct _ParrilladaFileMonitorSearchResult ParrilladaFileMonitorSearchResult;

static void
parrillada_inotify_file_data_free (ParrilladaInotifyFileData *data)
{
	g_free (data->name);
	g_free (data);
}
...
static void
parrillada_file_monitor_moved_to_event (ParrilladaFileMonitor *self,
				     gpointer callback_data,
				     const gchar *name,
				     guint32 cookie)
{
	ParrilladaInotifyMovedData *data = NULL;
	ParrilladaFileMonitorPrivate *priv;
	ParrilladaFileMonitorClass *klass;
	GSList *iter;

	priv = PARRILLADA_FILE_MONITOR_PRIVATE (self);
	klass = PARRILLADA_FILE_MONITOR_GET_CLASS (self);

	PARRILLADA_BURN_LOG ("File Monitoring (move to for %s)", name);
	...
	&&  data->type == PARRILLADA_FILE_MONITOR_FOLDER) {
		/* Simple renaming */
...
static gboolean
parrillada_file_monitor_move_timeout_cb (ParrilladaFileMonitor *self)
{
	ParrilladaInotifyMovedData *data;
	ParrilladaFileMonitorPrivate *priv;
	ParrilladaFileMonitorClass *klass;
	...
	PARRILLADA_BURN_LOG ("File Monitoring (move timeout for %s)", data->name);
...
static void
parrillada_file_monitor_moved_from_event (ParrilladaFileMonitor *self,
				       ParrilladaFileMonitorType type,
				       gpointer callback_data,
				       const gchar *name,
				       guint32 cookie)
{
	...
	PARRILLADA_BURN_LOG ("File Monitoring (moved from event for %s)", name);
	...
	data = g_new0 (ParrilladaInotifyMovedData, 1);
	data->type = type;
	data->cookie = cookie;
	...
	data->id = g_timeout_add_seconds (5,
					  (GSourceFunc) parrillada_file_monitor_move_timeout_cb,
					  self);
...
static void
parrillada_file_monitor_directory_event (ParrilladaFileMonitor *self,
				      ParrilladaFileMonitorType type,
				      gpointer callback_data,
				      const gchar *name,
				      struct inotify_event *event)
{
	...
	if (event->mask & IN_ATTRIB) {
		PARRILLADA_BURN_LOG ("File Monitoring (attributes changed for %s)", name);
		if (klass->file_modified)
			klass->file_modified (self, callback_data, name);
	}
	else if (event->mask & IN_MODIFY) {
		PARRILLADA_BURN_LOG ("File Monitoring (modified for %s)", name);
		if (klass->file_modified)
			klass->file_modified (self, callback_data, name);
	}
	else if (event->mask & IN_MOVED_FROM) {
		PARRILLADA_BURN_LOG ("File Monitoring (moved from for %s)", name);
		parrillada_file_monitor_moved_from_event (self, type, callback_data, name, event->cookie);
	}
	else if (event->mask & IN_MOVED_TO) {
		PARRILLADA_BURN_LOG ("File Monitoring (moved to for %s)", name);
		parrillada_file_monitor_moved_to_event (self, callback_data, name, event->cookie);
	}
	else if (event->mask & (IN_DELETE|IN_UNMOUNT)) {
		PARRILLADA_BURN_LOG ("File Monitoring (delete/unmount for %s)", name);
		if (klass->file_removed)
			klass->file_removed (self,
					     type,
					     callback_data,
		...
		PARRILLADA_BURN_LOG ("File Monitoring (create for %s)", name);
		if (klass->file_added)
			klass->file_added (self, callback_data, name);
	}
}
...
static void
parrillada_file_monitor_inotify_file_event (ParrilladaFileMonitor *self,
					 GSList *list,
					 const gchar *name,
					 struct inotify_event *event)
{
	ParrilladaInotifyFileData *data = NULL;
	ParrilladaFileMonitorPrivate *priv;
	ParrilladaFileMonitorClass *klass;
	GSList *iter;
	...
	if (event->mask & IN_MOVED_TO) {
		ParrilladaInotifyMovedData *moved_data = NULL;
		GSList *iter;
		...
		if (moved_data->type == PARRILLADA_FILE_MONITOR_FOLDER) {
			ParrilladaFileMonitorClass *klass;
	...
	if (event->mask & IN_MOVED_FROM) {
		parrillada_file_monitor_moved_from_event (self,
						       PARRILLADA_FILE_MONITOR_FILE,
						       data->callback_data,
						       name,
						       event->cookie);
	...
	parrillada_file_monitor_directory_event (self,
					      PARRILLADA_FILE_MONITOR_FILE,
					      data->callback_data,
					      NULL,
					      event);
...
static gboolean
parrillada_file_monitor_inotify_monitor_cb (GIOChannel *channel,
					 GIOCondition condition,
					 ParrilladaFileMonitor *self)
{
	ParrilladaFileMonitorPrivate *priv;
	struct inotify_event event;
	gpointer callback_data;
	...
	while (condition & G_IO_IN) {
		...
		if (!callback_data) {
			...
			list = callback_data;
			parrillada_file_monitor_inotify_file_event (self,
								 list,
								 name,
								 &event);
		}
...
static guint32
parrillada_file_monitor_start_monitoring_real (ParrilladaFileMonitor *self,
					    const gchar *uri)
{
	ParrilladaFileMonitorPrivate *priv;
	gchar *unescaped_uri;
	...
	if (wd == -1) {
		PARRILLADA_BURN_LOG ("ERROR creating watch for local file %s : %s\n",
				  path,
				  g_strerror (errno));
		g_free (path);
		return 0;
...
gboolean
parrillada_file_monitor_single_file (ParrilladaFileMonitor *self,
				  const gchar *uri,
				  gpointer callback_data)
{
	ParrilladaFileMonitorPrivate *priv;
	ParrilladaInotifyFileData *data;
	gchar *parent;
	GSList *list;
	GFile *file;
	guint32 wd;
	...
	wd = parrillada_file_monitor_start_monitoring_real (self, parent);
	g_free (parent);
	...
	data = g_new0 (ParrilladaInotifyFileData, 1);
	data->callback_data = callback_data;
...
gboolean
parrillada_file_monitor_directory_contents (ParrilladaFileMonitor *self,
				  	 const gchar *uri,
				       	 gpointer callback_data)
{
	ParrilladaFileMonitorPrivate *priv;
	 ...
	 * parent directory. We give them the same handle as their parent
	 * directory to find it more easily and mark it as being watched */
	wd = parrillada_file_monitor_start_monitoring_real (self, uri);
...
static void
parrillada_file_monitor_foreach_cancel_file_cb (gpointer key,
					     gpointer hash_data,
					     gpointer callback_data)
{
	GSList *iter;
	ParrilladaFileMonitorCancelForeach *data = callback_data;

	for (iter = hash_data; iter; iter = iter->next) {
		ParrilladaInotifyFileData *file_data = NULL;

		file_data = iter->data;
		if (data->func (file_data->callback_data, data->callback_data)) {
			ParrilladaFileMonitorSearchResult *result;

			result = g_new0 (ParrilladaFileMonitorSearchResult, 1);
			result->key = key;
...
static gboolean
parrillada_file_monitor_foreach_cancel_directory_cb (gpointer key,
						  gpointer hash_data,
						  gpointer callback_data)
{
				     ...
				     gpointer callback_data)
{
	...
	GSList *next;
	ParrilladaFileMonitorPrivate *priv;
	ParrilladaFileMonitorCancelForeach data;
	...
	g_hash_table_foreach (priv->files,
			      parrillada_file_monitor_foreach_cancel_file_cb,
			      &data);
	...
	for (iter = data.results; iter; iter = iter->next) {
		...
		parrillada_inotify_file_data_free (result->callback_data);
	...
	g_hash_table_foreach_remove (priv->directories,
				     parrillada_file_monitor_foreach_cancel_directory_cb,
				     &data);
...
static gboolean
parrillada_file_monitor_foreach_file_reset_cb (gpointer key,
					    gpointer hash_data,
					    gpointer callback_data)
{
	...
	/* really close the watch */
	g_slist_foreach (hash_data, (GFunc) parrillada_inotify_file_data_free, NULL);
...
static gboolean
parrillada_file_monitor_foreach_directory_reset_cb (gpointer key,
						 gpointer data,
						 gpointer callback_data)
{
	int dev_fd = GPOINTER_TO_INT (callback_data);
...
void
parrillada_file_monitor_reset (ParrilladaFileMonitor *self)
{
	ParrilladaFileMonitorPrivate *priv;

	priv = PARRILLADA_FILE_MONITOR_PRIVATE (self);
	g_hash_table_foreach_remove (priv->files,
				     parrillada_file_monitor_foreach_file_reset_cb,
				     GINT_TO_POINTER (g_io_channel_unix_get_fd (priv->notify)));

	g_hash_table_foreach_remove (priv->directories,
				     parrillada_file_monitor_foreach_directory_reset_cb,
				     GINT_TO_POINTER (g_io_channel_unix_get_fd (priv->notify)));
}

static void
parrillada_file_monitor_init (ParrilladaFileMonitor *object)
{
	ParrilladaFileMonitorPrivate *priv;
	int fd;

	priv = PARRILLADA_FILE_MONITOR_PRIVATE (object);
	...
	if (fd != -1) {
		...
		priv->notify_id = g_io_add_watch (priv->notify,
						  ...
						  (GIOFunc) parrillada_file_monitor_inotify_monitor_cb,
						  object);
...
static void
parrillada_file_monitor_finalize (GObject *object)
{
	ParrilladaFileMonitorPrivate *priv;

	priv = PARRILLADA_FILE_MONITOR_PRIVATE (object);

	parrillada_file_monitor_reset (PARRILLADA_FILE_MONITOR (object));
	...
	G_OBJECT_CLASS (parrillada_file_monitor_parent_class)->finalize (object);
}

static void
parrillada_file_monitor_class_init (ParrilladaFileMonitorClass *klass)
{
	GObjectClass* object_class = G_OBJECT_CLASS (klass);

	g_type_class_add_private (klass, sizeof (ParrilladaFileMonitorPrivate));

	object_class->finalize = parrillada_file_monitor_finalize;
}
