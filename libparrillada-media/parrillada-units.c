...
gchar *
parrillada_units_get_time_string_from_size (gint64 bytes,
					 gboolean with_unit,
					 gboolean round)
{
	guint64 time = 0;

	time = PARRILLADA_BYTES_TO_DURATION (bytes);
	return parrillada_units_get_time_string (time, with_unit, round);
}
