...
struct _ParrilladaModeSelectCDB {
	uchar opcode		:8;
...
typedef struct _ParrilladaModeSelectCDB ParrilladaModeSelectCDB;

PARRILLADA_SCSI_COMMAND_DEFINE (ParrilladaModeSelectCDB,
			     MODE_SELECT,
			     PARRILLADA_SCSI_WRITE);

#define PARRILLADA_MODE_DATA(data)			((ParrilladaScsiModeData *) (data))

ParrilladaScsiResult
parrillada_spc1_mode_select (ParrilladaDeviceHandle *handle,
			  ParrilladaScsiModeData *data,
			  int size,
			  ParrilladaScsiErrCode *error)
{
	ParrilladaModeSelectCDB *cdb;
...
//	PARRILLADA_SET_16 (data->hdr.len, 0);
//	PARRILLADA_SET_16 (data->hdr.bdlen, 0);
