...
typedef struct _ParrilladaReadCDCDB ParrilladaReadCDCDB;

PARRILLADA_SCSI_COMMAND_DEFINE (ParrilladaReadCDCDB,
			     READ_CD,
			 ...
			 ParrilladaScsiErrCode *error)
{
	...
	PARRILLADA_SET_32 (cdb->start_lba, start);

	/* NOTE: if we just want to test if block is readable len can be 0 */
	PARRILLADA_SET_24 (cdb->len, size);
