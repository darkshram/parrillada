...
static void
parrillada_sense_data_print (uchar *sense_data)
{
	int i;
...
parrillada_sense_data_unknown (uchar *sense_data, ParrilladaScsiErrCode *err)
{
	PARRILLADA_SCSI_SET_ERRCODE (err, PARRILLADA_SCSI_ERR_UNKNOWN);
	parrillada_sense_data_print (sense_data);

	return PARRILLADA_SCSI_FAILURE;
}

static ParrilladaScsiResult
parrillada_sense_data_not_ready (uchar *sense_data, ParrilladaScsiErrCode *err)
{
	ParrilladaScsiResult res = PARRILLADA_SCSI_FAILURE;

	switch (SENSE_DATA_ASC (sense_data)) {
		case ASC_CODE_NO_MEDIUM:
			/* No need to use PARRILLADA_SCSI_SET_ERRCODE
			 * as this is not necessarily an error */
			*err = PARRILLADA_SCSI_NO_MEDIUM;
			break;
		case ASC_CODE_NOT_READY:
			PARRILLADA_SCSI_SET_ERRCODE (err, PARRILLADA_SCSI_NOT_READY);
			break;

		default:
			res = parrillada_sense_data_unknown (sense_data, err);
			break;
	}
...
static ParrilladaScsiResult
parrillada_sense_data_illegal_request (uchar *sense_data, ParrilladaScsiErrCode *err)
{
	ParrilladaScsiResult res = PARRILLADA_SCSI_FAILURE;

	switch (SENSE_DATA_ASC_ASCQ (sense_data)) {
		case ASC_ASCQ_CODE_INVALID_COMMAND:
			PARRILLADA_SCSI_SET_ERRCODE (err, PARRILLADA_SCSI_INVALID_COMMAND);
			break;

		case ASC_ASCQ_CODE_OUTRANGE_ADDRESS:
			PARRILLADA_SCSI_SET_ERRCODE (err, PARRILLADA_SCSI_OUTRANGE_ADDRESS);
			break;

		case ASC_ASCQ_CODE_INVALID_ADDRESS:
			PARRILLADA_SCSI_SET_ERRCODE (err, PARRILLADA_SCSI_INVALID_ADDRESS);
			break;

		case ASC_ASCQ_CODE_INVALID_FIELD_IN_PARAM:
			PARRILLADA_SCSI_SET_ERRCODE (err, PARRILLADA_SCSI_INVALID_PARAMETER);
			break;

		case ASC_ASCQ_CODE_INVALID_FIELD_IN_CDB:
			PARRILLADA_SCSI_SET_ERRCODE (err, PARRILLADA_SCSI_INVALID_FIELD);
			break;

		case ASC_ASCQ_CODE_KEY_NOT_ESTABLISHED:
		case ASC_ASCQ_CODE_SCRAMBLED_SECTOR:
			PARRILLADA_SCSI_SET_ERRCODE (err, PARRILLADA_SCSI_KEY_NOT_ESTABLISHED);
			break;

		case ASC_ASCQ_CODE_INVALID_TRACK_MODE:
			PARRILLADA_SCSI_SET_ERRCODE (err, PARRILLADA_SCSI_INVALID_TRACK_MODE);
			break;
...
static ParrilladaScsiResult
parrillada_sense_data_unit_attention (uchar *sense_data,
				   ParrilladaScsiErrCode *err)
{
	ParrilladaScsiResult res = PARRILLADA_SCSI_FAILURE;

	switch (SENSE_DATA_ASC_ASCQ (sense_data)) {
		case ASC_ASCQ_CODE_INSUFFICIENT_TIME_FOR_OPERATION:
			PARRILLADA_SCSI_SET_ERRCODE (err, PARRILLADA_SCSI_TIMEOUT);
			break;
...
parrillada_sense_data_process (uchar *sense_data,
			    ParrilladaScsiErrCode *err)
{
	ParrilladaScsiResult res = PARRILLADA_SCSI_FAILURE;
	...
	switch (SENSE_DATA_KEY (sense_data)) {
		case SENSE_CODE_NOT_READY:
			res = parrillada_sense_data_not_ready (sense_data, err);
			break;

		case SENSE_CODE_ILLEGAL_REQUEST:
			res = parrillada_sense_data_illegal_request (sense_data, err);
			break;

		case SENSE_CODE_UNIT_ATTENTION:
			res = parrillada_sense_data_unit_attention (sense_data, err);
			break;
