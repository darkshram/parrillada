...
typedef enum {
	PARRILLADA_ISO_FLAG_NONE	= 0,
	PARRILLADA_ISO_FLAG_RR	= 1
} ParrilladaIsoFlag;

gboolean
parrillada_iso9660_is_primary_descriptor (const gchar *buffer,
				       GError **error);
...
ParrilladaVolFile *
parrillada_iso9660_get_contents (ParrilladaVolSrc *src,
			      const gchar *block,
			      gint64 *nb_blocks,
			      GError **error);
...
ParrilladaVolFile *
parrillada_iso9660_get_file (ParrilladaVolSrc *src,
			  const gchar *path,
			  const gchar *block,
			  GError **error);
