...
struct _ParrilladaScsiMechStatusCDB {
	uchar opcode;
...
typedef struct _ParrilladaScsiMechStatusCDB ParrilladaScsiMechStatusCDB;

PARRILLADA_SCSI_COMMAND_DEFINE (ParrilladaScsiMechStatusCDB,
			     MECH_STATUS,
...
{
	ParrilladaScsiMechStatusCDB *cdb;
	...
	PARRILLADA_SET_16 (cdb->len, sizeof (ParrilladaScsiMechStatusHdr));

	memset (hdr, 0, sizeof (ParrilladaScsiMechStatusHdr));
	res = parrillada_scsi_command_issue_sync (cdb,
					       hdr,
					       sizeof (ParrilladaScsiMechStatusHdr),
					       error);
