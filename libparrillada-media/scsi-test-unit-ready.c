...
struct _ParrilladaTestUnitReadyCDB {
	uchar opcode;
	uchar reserved		[4];
	uchar ctl;
};

typedef struct _ParrilladaTestUnitReadyCDB ParrilladaTestUnitReadyCDB;

PARRILLADA_SCSI_COMMAND_DEFINE (ParrilladaTestUnitReadyCDB,
			     TEST_UNIT_READY,
			     PARRILLADA_SCSI_READ);

ParrilladaScsiResult
parrillada_spc1_test_unit_ready (ParrilladaDeviceHandle *handle,
			      ParrilladaScsiErrCode *error)
{
	ParrilladaTestUnitReadyCDB *cdb;
