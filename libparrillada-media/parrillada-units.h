...
#ifndef _PARRILLADA_UNITS_H_
#define _PARRILLADA_UNITS_H_
...
#define PARRILLADA_SPEED_TO_RATE_CD(speed)						\
	(guint) ((speed) * CD_RATE)

#define PARRILLADA_SPEED_TO_RATE_DVD(speed)					\
	(guint) ((speed) * DVD_RATE)

#define PARRILLADA_SPEED_TO_RATE_BD(speed)						\
	(guint) ((speed) * BD_RATE)

#define PARRILLADA_RATE_TO_SPEED_CD(rate)						\
	(gdouble) ((gdouble) (rate) / (gdouble) CD_RATE)

#define PARRILLADA_RATE_TO_SPEED_DVD(rate)						\
	(gdouble) ((gdouble) (rate) / (gdouble) DVD_RATE)

#define PARRILLADA_RATE_TO_SPEED_BD(rate)						\
...
/**
 * PARRILLADA_SECTORS_TO_DURATION:
 * This macro is used to convert sector sizes in
 * in length (in nanoseconds).
 *
**/
#define PARRILLADA_SECTORS_TO_DURATION(sectors)	\
...
gchar *
parrillada_units_get_time_string (guint64 time,
			       gboolean with_unit,
			       gboolean round);

gchar *
parrillada_units_get_time_string_from_size (gint64 size,
					 gboolean with_unit,
					 gboolean round);

G_END_DECLS

#endif /* _PARRILLADA_UNITS_H_ */
