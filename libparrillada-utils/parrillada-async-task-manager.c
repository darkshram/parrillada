...
static void parrillada_async_task_manager_class_init (ParrilladaAsyncTaskManagerClass *klass);
static void parrillada_async_task_manager_init (ParrilladaAsyncTaskManager *sp);
static void parrillada_async_task_manager_finalize (GObject *object);

struct ParrilladaAsyncTaskManagerPrivate {
...
struct _ParrilladaAsyncTaskCtx {
	ParrilladaAsyncPriority priority;
	const ParrilladaAsyncTaskType *type;
	GCancellable *cancel;
	gpointer data;
};
typedef struct _ParrilladaAsyncTaskCtx ParrilladaAsyncTaskCtx;
...
GType
parrillada_async_task_manager_get_type ()
{
	static GType type = 0;

	if(type == 0) {
		static const GTypeInfo our_info = {
			sizeof (ParrilladaAsyncTaskManagerClass),
			NULL,
			NULL,
			(GClassInitFunc)parrillada_async_task_manager_class_init,
			NULL,
			NULL,
			sizeof (ParrilladaAsyncTaskManager),
			0,
			(GInstanceInitFunc)parrillada_async_task_manager_init,
		};

		type = g_type_register_static (G_TYPE_OBJECT,
					       "ParrilladaAsyncTaskManager",
					       &our_info,
					       0);
	}
...
static void
parrillada_async_task_manager_class_init (ParrilladaAsyncTaskManagerClass *klass)
{
	GObjectClass *object_class = G_OBJECT_CLASS (klass);

	parent_class = g_type_class_peek_parent (klass);
	object_class->finalize = parrillada_async_task_manager_finalize;
}

static void
parrillada_async_task_manager_init (ParrilladaAsyncTaskManager *obj)
{
	obj->priv = g_new0 (ParrilladaAsyncTaskManagerPrivate, 1);
...
static void
parrillada_async_task_manager_finalize (GObject *object)
{
	ParrilladaAsyncTaskManager *cobj;

	cobj = PARRILLADA_ASYNC_TASK_MANAGER (object);
...
static void
parrillada_async_task_manager_insert_task (ParrilladaAsyncTaskManager *self,
					ParrilladaAsyncTaskCtx *ctx)
{
	GSList *iter;
	GSList *node;
	ParrilladaAsyncTaskCtx *tmp;
...
static gpointer
parrillada_async_task_manager_thread (ParrilladaAsyncTaskManager *self)
{
	gboolean result;
	GCancellable *cancel;
	ParrilladaAsyncTaskCtx *ctx;
	...
	while (1) {
		...
		ctx->cancel = cancel;
		ctx->priority &= ~PARRILLADA_ASYNC_RESCHEDULE;
		...
		if (!g_cancellable_is_cancelled (cancel)) {
			if (res == PARRILLADA_ASYNC_TASK_RESCHEDULE) {
				if (self->priv->waiting_tasks) {
					ParrilladaAsyncTaskCtx *next;

					next = self->priv->waiting_tasks->data;
					if (next->priority > ctx->priority)
						parrillada_async_task_manager_insert_task (self, ctx);
					else
...
gboolean
parrillada_async_task_manager_queue (ParrilladaAsyncTaskManager *self,
				  ParrilladaAsyncPriority priority,
				  const ParrilladaAsyncTaskType *type,
				  gpointer data)
{
	...
	ctx = g_new0 (ParrilladaAsyncTaskCtx, 1);
	ctx->priority = priority;
	ctx->type = type;
	ctx->data = data;

	g_mutex_lock (self->priv->lock);
	if (priority == PARRILLADA_ASYNC_IDLE)
		self->priv->waiting_tasks = g_slist_append (self->priv->waiting_tasks, ctx);
	else if (priority == PARRILLADA_ASYNC_NORMAL)
		parrillada_async_task_manager_insert_task (self, ctx);
	else if (priority == PARRILLADA_ASYNC_URGENT)
		self->priv->waiting_tasks = g_slist_prepend (self->priv->waiting_tasks, ctx);
	...
	if (self->priv->unused_threads) {
		...
		thread = g_thread_create ((GThreadFunc) parrillada_async_task_manager_thread,
					  self,
					  FALSE,
					  &error);
		if (!thread) {
...
gboolean
parrillada_async_task_manager_foreach_active (ParrilladaAsyncTaskManager *self,
...
gboolean
parrillada_async_task_manager_foreach_active_remove (ParrilladaAsyncTaskManager *self,
						  ParrilladaAsyncFindTask func,
						  gpointer user_data)
{
...
gboolean
parrillada_async_task_manager_find_urgent_task (ParrilladaAsyncTaskManager *self,
					     ParrilladaAsyncFindTask func,
					     gpointer user_data)
{
	GSList *iter;
	...
	for (iter = self->priv->waiting_tasks; iter; iter = iter->next) {
		...
		if (func (self, ctx->data, user_data)) {
			ctx->priority = PARRILLADA_ASYNC_URGENT;
