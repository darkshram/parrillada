...
GQuark
parrillada_utils_error_quark (void)
{
	static GQuark quark = 0;

	if (!quark)
		quark = g_quark_from_static_string ("Parrillada_utils_error");
...
static gboolean
parrillada_utils_clear_strings_cb (gchar *string,
				guint ref,
				gpointer NULL_data)
{
	g_free (string);
...
static void
parrillada_utils_free (void)
{
	if (stringsH) {
		G_LOCK (stringsH);
		g_hash_table_foreach_remove (stringsH,
					     (GHRFunc) parrillada_utils_clear_strings_cb,
					     NULL);
...
{
	g_atexit (parrillada_utils_free);
}

gchar*
parrillada_utils_validate_utf8 (const gchar *name)
{
	gchar *retval, *ptr;
	const gchar *invalid;
...
parrillada_utils_get_uri_name (const gchar *uri)
{
	...
	utf8_name = parrillada_utils_validate_utf8 (name);
	if (utf8_name) {
		g_free (name);
		return utf8_name;
	}
...
gchar *
parrillada_utils_register_string (const gchar *string)
{
	gboolean success;
	gpointer key, reftmp;
	guint ref;
...
void
parrillada_utils_unregister_string (const gchar *string)
{
	gboolean success;
	gpointer key, reftmp;
	guint ref;
...
GtkWidget *
parrillada_utils_properties_get_label (GtkWidget *properties)
{
	GList *children;
	GList *iter;
...
GtkWidget *
parrillada_utils_pack_properties_list (const gchar *title, GSList *list)
{
...
GtkWidget *
parrillada_utils_pack_properties (const gchar *title, ...)
{
	va_list vlist;
	GtkWidget *child;
	GtkWidget *result;
	...
	result = parrillada_utils_pack_properties_list (title, list);
	g_slist_free (list);
...
GtkWidget *
parrillada_utils_make_button (const gchar *text,
			   const gchar *stock,
			   const gchar *theme, 
			   GtkIconSize size)
{
...
GtkWidget *
parrillada_utils_create_message_dialog (GtkWidget *parent,
				     ...
				     GtkMessageType type)
{
	...
	gtk_window_set_icon_name (GTK_WINDOW (message),
	                          parent? gtk_window_get_icon_name (GTK_WINDOW (parent)):"parrillada");
...
void
parrillada_utils_message_dialog (GtkWidget *parent,
			      const gchar *primary_message,
			      const gchar *secondary_message,
			      GtkMessageType type)
{
	...
	message = parrillada_utils_create_message_dialog (parent,
						       primary_message,
						       secondary_message,
						       type);
