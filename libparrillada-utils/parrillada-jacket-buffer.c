...
typedef struct _ParrilladaJacketBufferPrivate ParrilladaJacketBufferPrivate;
struct _ParrilladaJacketBufferPrivate
{
	GSList *tags;
...
#define PARRILLADA_JACKET_BUFFER_PRIVATE(o)  (G_TYPE_INSTANCE_GET_PRIVATE ((o), PARRILLADA_TYPE_JACKET_BUFFER, ParrilladaJacketBufferPrivate))

#define PARRILLADA_JACKET_BUFFER_TAG	"jacket-buffer-tag"

G_DEFINE_TYPE (ParrilladaJacketBuffer, parrillada_jacket_buffer, GTK_TYPE_TEXT_BUFFER);
...
{
	ParrilladaJacketBufferPrivate *priv;

	priv = PARRILLADA_JACKET_BUFFER_PRIVATE (self);
...
static void
parrillada_jacket_buffer_cursor_position_changed_cb (GObject *buffer,
						  GParamSpec *spec,
						  gpointer NULL_data)
{
	ParrilladaJacketBufferPrivate *priv;
	...
	priv = PARRILLADA_JACKET_BUFFER_PRIVATE (buffer);
...
static void
parrillada_jacket_buffer_insert_text (GtkTextBuffer *buffer,
				   ...
				   gint length)
{
	...
	GTK_TEXT_BUFFER_CLASS (parrillada_jacket_buffer_parent_class)->insert_text (buffer, location, text, length);
					 ...
					 gboolean show)
{
	...
	if (show) {
		...
		GTK_TEXT_BUFFER_CLASS (parrillada_jacket_buffer_parent_class)->insert_text (GTK_TEXT_BUFFER (self), &start, priv->default_text, -1);

		gtk_text_buffer_get_start_iter (GTK_TEXT_BUFFER (self), &start);
		gtk_text_buffer_get_end_iter (GTK_TEXT_BUFFER (self), &end);
		gtk_text_buffer_apply_tag_by_name (GTK_TEXT_BUFFER (self),
						   PARRILLADA_JACKET_BUFFER_TAG,
						   &start, &end);
		priv->empty = 1;
	}
	else if (priv->empty) {
					...
					const gchar *default_text)
{
	...
	if (!gtk_text_tag_table_lookup (table, PARRILLADA_JACKET_BUFFER_TAG))
	...
	gtk_text_buffer_get_start_iter (GTK_TEXT_BUFFER (self), &start);
	GTK_TEXT_BUFFER_CLASS (parrillada_jacket_buffer_parent_class)->insert_text (GTK_TEXT_BUFFER (self), &start, default_text, -1);
...
static void
parrillada_jacket_buffer_init (ParrilladaJacketBuffer *object)
{
	ParrilladaJacketBufferPrivate *priv;

	priv = PARRILLADA_JACKET_BUFFER_PRIVATE (object);

	priv->empty = 1;
	g_signal_connect (object,
			  "notify::cursor-position",
			  G_CALLBACK (parrillada_jacket_buffer_cursor_position_changed_cb),
			  NULL);
}

static void
parrillada_jacket_buffer_finalize (GObject *object)
{
	ParrilladaJacketBufferPrivate *priv;
	...
	G_OBJECT_CLASS (parrillada_jacket_buffer_parent_class)->finalize (object);
}

static void
parrillada_jacket_buffer_class_init (ParrilladaJacketBufferClass *klass)
{
	GObjectClass* object_class = G_OBJECT_CLASS (klass);
	GtkTextBufferClass* parent_class = GTK_TEXT_BUFFER_CLASS (klass);

	g_type_class_add_private (klass, sizeof (ParrilladaJacketBufferPrivate));

	object_class->finalize = parrillada_jacket_buffer_finalize;

	parent_class->insert_text = parrillada_jacket_buffer_insert_text;
}

ParrilladaJacketBuffer *
parrillada_jacket_buffer_new (void)
{
	return g_object_new (PARRILLADA_TYPE_JACKET_BUFFER, NULL);
}
